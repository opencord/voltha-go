...
// ProveQueryType typed values for query types
type ProbeQueryType int
...
func (p *Probe) ListenAndServe(port int) {
	...
	mux.HandleFunc("/readz", func(w http.ResponseWriter, req *http.Request) {
		...
		p.query <- ProbeQuery{ProbeQueryReady, w, done}
		select {
		case <-done:
		}
	})
	...
	mux.HandleFunc("/healthz", func(w http.ResponseWriter, req *http.Request) {
		...
		p.query <- ProbeQuery{ProbeQueryHealth, w, done}
		select {
		case <-done:
		}
	})
	...
	mux.HandleFunc("/detailz", func(w http.ResponseWriter, req *http.Request) {
		...
		p.query <- ProbeQuery{ProbeQueryDetail, w, done}
		select {
		case <-done:
		}
...
func (p *Probe) UpdateListener() {
	for {
		select {
		case service := <-p.register:
			if _, ok := p.status[service]; !ok {
				p.status[service] = ServiceStatusUnknown
			}
		case query := <-p.query:
			switch query.Type {
			case ProbeQueryReady:
				if p.isReady {
					query.Response.WriteHeader(http.StatusOK)
				} else {
					query.Response.WriteHeader(http.StatusTeapot)
				}
				query.Done <- true
			case ProbeQueryHealth:
				if p.isHealthy {
					query.Response.WriteHeader(http.StatusOK)
				} else {
					query.Response.WriteHeader(http.StatusTeapot)
				...
				query.Done <- true
			case ProbeQueryDetail:
				query.Response.Write([]byte("{"))
				comma := ""
				for c, s := range p.status {
					query.Response.Write([]byte(fmt.Sprintf("%s\"%s\": \"%s\"", comma, c, s.String())))
					comma = ", "
				}
				query.Response.Write([]byte("}"))
				query.Response.Header().Set("Content-Type", "application/json")
				query.Response.WriteHeader(http.StatusOK)
				query.Done <- true
			default:
				query.Response.WriteHeader(http.StatusNotFound)
				query.Done <- true
			}
