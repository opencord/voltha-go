...
import (
	...
	openolt_pb "github.com/opencord/voltha-go/protos/openolt"
)
...
/* TODO : tmp: remove it later after integrated with openolt resource mgr */
...
func NewTechProfileMgr(resourceMgr *ResourceMgr) (*TechProfileMgr,error){
	var techprofileObj TechProfileMgr
        log.Debug("Creating techprofile Manager")
	...
	techprofileObj.config.KVBackend = techprofileObj.SetKVClient()
        ...
        log.Info("Success creating tech profile manager instance")
...
func(t *TechProfileMgr) GetTechProfileInstanceKVPath(techProfiletblID uint32 , uniPortName string)string{
	return fmt.Sprintf(t.config.TPInstanceKVPath,t.resourceMgr.technology, techProfiletblID, uniPortName)
}
...
func (t *TechProfileMgr) getTPFromKVStore(techProfiletblID uint32 )(*DefaultTechProfile){
	...
	key := fmt.Sprintf(t.config.TPFileKVPath,t.resourceMgr.technology,techProfiletblID)
...
func (t *TechProfileMgr) CreateTechProfInstance(techProfiletblID uint32, uniPortName string, intfId uint32) (*TechProfile){
	...
	tpInstance = allocateTPInstance(uniPortName, tp, intfId, t.config.DefaultNumTconts)
...
func allocateTPInstance(uniPortName string, tp *DefaultTechProfile, intfId uint32, numOfTconts uint32 )(*TechProfile){

	var usGemPortAttributeList []iGemPortAttribute
	var dsGemPortAttributeList []iGemPortAttribute

        log.Info("Allocating TechProfileMgr instance from techprofile template")
        if numOfTconts > 1 {
             log.Errorw("Multiple Tconts not supported currently",log.Fields{"uniPortName":uniPortName,"intfId":intfId})
             return nil
        }
	// TODO : HARCODED NOW , Uncomment  during rsrc mgmr integration
	//alloc_id := t.resourceMgr.GetResourceId(intfId,"ALLOC_ID",numTconts)
	var tcontID uint32 = 1024
	// TODO : HARCODED NOW , Uncomment  during rsrc mgmr integration
	// validate number of gem ports allocated 
	//gemPorts := t.resourceMgr.GetResourceId(intfId,"GEMPORT_ID",tp.numGemPorts)
        gemPorts := make([]uint32,10,10)
        fmt.Println("Num GEM ports in TP:",tp.NumGemPorts)
        var index uint32
        for index=0 ; index < tp.NumGemPorts;index++ {
            gemPorts[index]= 2024+index
        }
...
func(t *TechProfileMgr) getDefaultTechProfile()*DefaultTechProfile {
	...
	return &DefaultTechProfile{
		...
		ProfileType: t.resourceMgr.technology,
