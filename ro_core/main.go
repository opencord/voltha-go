...
func (ro *roCore) start(ctx context.Context) {
	...
	if err == nil {
		...
		c.SetTransactionContext(ro.config.InstanceID,
			"service/voltha/transactions/",
			ro.kvClient,
			ro.config.KVStoreTimeout,
			ro.config.KVTxnKeyDelTime)
	}

	// Setup Kafka Client
	if ro.kafkaClient, err = newKafkaClient("sarama", ro.config.KafkaAdapterHost, ro.config.KafkaAdapterPort, ro.config.InstanceID); err != nil {
		log.Fatal("Unsupported-kafka-client")
	}

	// Create the core service
	ro.core = c.NewCore(ro.config.InstanceID, ro.config, ro.kvClient, ro.kafkaClient)
