...
const (
	...
	default_LiveProbeInterval     = time.Duration(60) * time.Second
	default_NotLiveProbeInterval  = time.Duration(5) * time.Second // Probe more frequently to detect Recovery early
)
...
func (cf *ROCoreFlags) ParseCommandArguments() {
	...
	flag.Var(&TimeDurationValue{&(cf.LiveProbeInterval)}, "live_probe_interval", help)

	help = fmt.Sprintf("Time interval between liveness probes while in a not live state")
	flag.Var(&TimeDurationValue{&(cf.NotLiveProbeInterval)}, "not_live_probe_interval", help)
...
func (tdv TimeDurationValue) String() string {
	if tdv.duration != nil {
		return tdv.duration.String()
	}
	return ""
}

func (tdv TimeDurationValue) Set(s string) error {
