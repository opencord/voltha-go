...
func (mpMgr *ModelProxyManager) GetEventFilter(ctx context.Context, id string) (*voltha.EventFilter, error) {
	...
	if eventFilter, err := agent.Get(id); err != nil {
		return nil, status.Errorf(codes.Internal, err.Error())
	} else if eventFilter != nil {
		_, ok := eventFilter.(*voltha.EventFilter)
		if !ok {
			return nil, status.Errorf(codes.Internal, MultipleValuesMsg,
				id, EventFilters.String())
		}
		return eventFilter.(*voltha.EventFilter), nil
	}
	return &voltha.EventFilter{}, status.Errorf(codes.NotFound, "event-filter-%s", id)
}
