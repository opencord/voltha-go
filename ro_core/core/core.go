...
func NewCore(id string, cf *config.ROCoreFlags, kvClient kvstore.Client, kafkaClient kafka.Client) *Core {
	var core Core
	core.instanceId = id
...
func (core *Core) Start(ctx context.Context) {
	...
	core.genericMgr = newCommonManager(core.clusterDataProxy)
