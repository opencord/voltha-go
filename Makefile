...
rw_core_profile: local-protos local-lib-go
	docker build $(DOCKER_BUILD_ARGS) -t ${RWCORE_IMAGENAME}:${DOCKER_TAG}-profile -t ${RWCORE_IMAGENAME}:latest -f docker/Dockerfile.rw_core_profile .
...
sca:
	...
	@${GOLANGCI_LINT} run --deadline=4m -E golint --out-format junit-xml ./rw_core/... ./tests/... ./common/... ./db/... | tee ./sca-report/sca-report.xml
