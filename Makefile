...
test-go-mod:
	git status > /dev/null
	git diff-index --quiet HEAD -- || (echo "Staged files must be committed before running this test" && exit 1)
	[[ `git ls-files --exclude-standard --others` == "" ]] || (echo "Untracked files must be cleaned up before running this test" && exit 1)
