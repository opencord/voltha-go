...
func (ep *EPTest) populateBackend() error {
	...
	for i := 0; i < numReplicas; i++ {
		...
		adapterKVKey := fmt.Sprintf("%s_%d", adapterID, i)
...
func (ep *EPTest) testEndpointManagerAPIs(t *testing.T, tm EndpointManager, serviceID string, deviceType string, replicas int) {
	...
	for deviceID := range deviceIDs {
		...
		for k := 0; k < replicas; k++ {
			owned, err := tm.IsDeviceOwnedByService(deviceID, deviceType, int32(k))
			if err != nil {
...
func TestEndpointManagerSuite(t *testing.T) {
	...
	err := tm.Start()
	if err != nil {
		logger.Fatalw("failed to start endpoint mamanger", log.Fields{"error": err})
	}

	defer tmt.stopAll()

	//1. Test APIs with multiple replicas
	tmt.testEndpointManagerAPIs(t, tm, "brcm_openomci_onu", "brcm_openomci_onu", tmt.maxReplicas)

	//2. Test APIs with single replica
	tmt.testEndpointManagerAPIs(t, tm, "openolt", "openolt", tmt.minReplicas)
}
