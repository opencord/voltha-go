...
type EndpointManager interface {
	// Start loads the services into cache.  For now the services are adapters with the device types
	Start() error

	// Stop ends the manager.
	Stop() error

	// GetEndpoint is called to get the endpoint to communicate with for a specific device and deviceType.  For
...
func (ep *endpointManager) Start() error {
	needToStart := false
	if ep.startOnce.Do(func() { needToStart = true }); !needToStart {
		return nil
	}
...
func (ep *endpointManager) Stop() error {
