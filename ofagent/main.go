...
func (e Main) shutdown_components() {
	fmt.Println("shutdown_components")
	e.exiting = true
...
func (e Main) start() {
	...
	go func() {
		fmt.Println("Interuption of ofagent - begin graceful shutdown")
		sig := <-gracefulStop
	...
	// add a main loop now for testing
	for {
	...
	visitor := func(a *flag.Flag) {
		if a.Name == "c" || a.Name == "-config" {
			if _, err := os.Stat(e.configPath); err == nil {
				...
				e.progArgErrors(fmt.Sprintf("Path doesn't exist: %+v", err.Error()))
			} else {
				e.progArgErrors(fmt.Sprintf("File exists issue: %+v", err.Error()))
			}
		}
