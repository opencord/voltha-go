...
func init() {
	...
	if BenchmarkProxyDeviceProxy, err = BenchmarkProxyRoot.node.CreateProxy(context.Background(), "/", false); err != nil {
	...
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			...
			if len(BenchmarkProxyPLT.addedDevices) > 0 {
				...
				firmProxy, err := BenchmarkProxyRoot.node.CreateProxy(context.Background(), "/", false)
				if err != nil {
	...
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			if len(BenchmarkProxyPLT.addedDevices) > 0 {
				...
				flowsProxy, err := BenchmarkProxyRoot.node.CreateProxy(context.Background(), "/devices/"+randomID+"/flows", false)
				if err != nil {
