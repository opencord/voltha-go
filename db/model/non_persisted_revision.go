...
// GetNode -
func (npr *NonPersistedRevision) GetNode() *node { // nolint
	npr.mutex.Lock()
	defer npr.mutex.Unlock()
	...
	if depth != 0 {
		...
		for fieldName, field := range ChildrenFields(latestRev.GetData()) {
			...
			if field.IsContainer {
				...
				if revs := npr.GetBranch().GetLatest().GetChildren(fieldName); revs != nil && len(revs) > 0 { // nolint
					rev := revs[0]
					if rev != nil {
						childData := rev.Get(depth - 1)
