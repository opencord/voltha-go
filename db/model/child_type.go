...
func ChildrenFields(cls interface{}) map[string]*ChildType {
	...
	if getChildTypes().GetCache() == nil {
		getChildTypes().ResetCache()
	}

	msgType := reflect.TypeOf(cls)
	inst := getChildTypes()

	if names, namesExist = inst.GetCacheEntry(msgType.String()); !namesExist {
		...
		getChildTypes().SetCacheEntry(msgType.String(), names)

	} else {
		log.Debugf("Cache entry for %s: %+v", msgType.String(), inst.GetCache()[msgType.String()])
	}
