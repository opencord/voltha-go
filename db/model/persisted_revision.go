		...
		case kvstore.PUT:
			...
			if err := proto.Unmarshal(event.Value.([]byte), data.Interface().(proto.Message)); err != nil {
				...
				if latestRev.GetNode().GetProxy() != nil {
					...
					_, err = latestRev.GetNode().GetProxy().GetRoot().KvStore.Client.Reserve(pathLock+"_", uuid.New().String(), ReservationTTL)
					if err != nil {
					...
					// Release path
					err = latestRev.GetNode().GetProxy().GetRoot().KvStore.Client.ReleaseReservation(pathLock + "_")
					if err != nil {
...
func (pr *PersistedRevision) LoadFromPersistence(ctx context.Context, path string, txid string, blobs map[string]*kvstore.KVPair) []Revision {
	...
	if pr.kvStore != nil && path != "" {
		if blobs == nil || len(blobs) == 0 { // nolint
