...
class GrpcClient(object):
    ...
    def update_meter_mod_table(self, device_id, meter_mod):
        log.info('In update_meter_mod_table grpc')
        req = MeterModUpdate(
            id=device_id,
            meter_mod=meter_mod
        )
        res = yield threads.deferToThread(
            ...
            metadata=((self.core_group_id_key, self.core_group_id),
                      self.get_core_transaction_metadata(),))
        log.info('update_meter_mod_table grpc done')
        returnValue(res)
    ...
    def list_meters(self, device_id):
        log.info('In list_meters')
        req = ID(id=device_id)
        res = yield threads.deferToThread(
            self.grpc_stub.ListLogicalDeviceMeters, req)
        log.info('done stat query', resp=res)
        returnValue(res.items)
