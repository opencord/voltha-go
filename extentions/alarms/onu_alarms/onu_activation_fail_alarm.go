...
type OnuActivationFailAlarm struct {
	AlarmData    ab.AlarmData
	AlarmContext map[string]string
}

func GetNewOnuActivationFailAlarm() OnuActivationFailAlarm {
	var los OnuActivationFailAlarm
	return los
}

func (onuAlarm OnuActivationFailAlarm) GetAlarmData(status bool, deviceId string) ab.AlarmData {
	var alarmData ab.AlarmData
	alarmData.Ts = float32(time.Now().UnixNano())
	alarmData.Description = FormatDescription("ONU_ACTIVATION_FAILED_ALARM", true)
	alarmData.Id = FormatId("ONU_ACTIVATION_FAILED_ALARM")
	...
	alarmData.Name = "ONU_ACTIVATION_FAILED_ALARM"

	return alarmData
}

func (onuAlarm OnuActivationFailAlarm) GetContextData(onuId uint32, intfId string) map[string]string {
...
func (onuAlarm OnuActivationFailAlarm) FormatDescription(alarmName string, status bool) string {
	if status {
...
func (onuAlarm OnuActivationFailAlarm) FormatId(alarmName string) string {
	return fmt.Sprintf("Voltha.openolt.%s.%s", alarmName, string(time.Now().Nanosecond()))
}

func (onuAlarm OnuActivationFailAlarm) RaiseAlarm(kc kafka.Client, topic kafka.Topic) error {
	ae := ab.CreateAlarmEvent(&onuAlarm.AlarmData, onuAlarm.AlarmContext)
...
func (onuAlarm OnuActivationFailAlarm) ClearAlarm(kc kafka.Client, topic kafka.Topic) error {
	ae := ab.CreateAlarmEvent(&onuAlarm.AlarmData, onuAlarm.AlarmContext)
	if err := ab.SendAlarm(kc, topic, ae); err != nil {
		log.Errorw("Failed to send alarm to kafka", log.Fields{"alarm_event": ae, "topic": topic.Name, "error": err})
		return err
