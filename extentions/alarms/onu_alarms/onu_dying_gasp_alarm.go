...
type DyingGaspAlarm struct {
	AlarmData    ab.AlarmData
	AlarmContext map[string]string
}

func GetNewDyingGaspAlarm() DyingGaspAlarm {
	var dgi DyingGaspAlarm
	return dgi
}

func (dgiAlarm DyingGaspAlarm) GetAlarmData(status bool, deviceId string) ab.AlarmData {
	var alarmData ab.AlarmData
	alarmData.Ts = float32(time.Now().UnixNano())
	alarmData.Description = FormatDescription("DYING_GASP_ALARM", true)
	alarmData.Id = FormatId("DYING_GASP_ALARM")
	...
	alarmData.Name = "DYING_GASP_ALARM"

	return alarmData
}

func (dgiAlarm DyingGaspAlarm) GetContextData(onuId uint32, intfId uint32, deviceId string) map[string]string {
...
func FormatDescription(alarmName string, status bool) string {
	if status {
...
func FormatId(alarmName string) string {
	return fmt.Sprintf("Voltha.openolt.%s.%s", alarmName, string(time.Now().Nanosecond()))
}

func (dgiAlarm DyingGaspAlarm) RaiseAlarm(kc kafka.Client, topic kafka.Topic) error {
	ae := ab.CreateAlarmEvent(&dgiAlarm.AlarmData, dgiAlarm.AlarmContext)
...
func (dgiAlarm DyingGaspAlarm) ClearAlarm(kc kafka.Client, topic kafka.Topic) error {
	ae := ab.CreateAlarmEvent(&dgiAlarm.AlarmData, dgiAlarm.AlarmContext)
	if err := ab.SendAlarm(kc, topic, ae); err != nil {
		log.Errorw("Failed to send alarm to kafka", log.Fields{"alarm_event": ae, "topic": topic.Name, "error": err})
		return err
