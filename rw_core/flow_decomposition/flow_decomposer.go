...
func (fd *FlowDecomposer) processControllerBoundFlow(agent coreIf.LogicalDeviceAgent, route []graph.RouteHop,
	...
	if agent.GetDeviceGraph().IsRootPort(inPortNo) {
		...
		for _, inputPort := range inPorts {
			...
			// Downstream trap flows are not required now
			// Downstream flow
			/*fa = &fu.FlowArgs{
				KV: fu.OfpFlowModArgs{"priority": uint64(flow.Priority), "meter_id": uint64(meterId), "write_metadata": metadataFromwriteMetadata},
				MatchFields: []*ofp.OfpOxmOfbField{
