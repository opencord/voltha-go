...
func (rq *RequestQueue) Start() {
	go func() {
		for {
			...
			// If the request is waiting then closing the channel will trigger the request to proceed.  Otherwise,
			...
			// Wait until that request is completed before letting another request go through
			<-rq.requestCompleteIndication
		}
	}()
}
