...
func newLogicalDeviceAgent(id string, deviceID string, ldeviceMgr *LogicalDeviceManager,
	...
	agent.defaultTimeout = time.Duration(timeout) * time.Millisecond
	agent.requestQueue = make(chan chan struct{}, maxOrderedRequestQueueSize)
	agent.requestComplete = make(chan struct{})
	return &agent
}
	...
	agent.startOnce.Do(func() {
		if !loadFromdB {
			...
			if added == nil {
				...
				log.Debugw("logicaldevice-created", log.Fields{"logicaldeviceId": agent.logicalDeviceID, "oltID": ld.RootDeviceId})
			}
			...
			if err != nil {
				returnErr = status.Errorf(codes.NotFound, "logical_device-%s", agent.logicalDeviceID)
				return
			}
		...
		if agent.ldProxy != nil {
			...
			returnErr = status.Error(codes.Internal, "logical-device-proxy-null")
			return
		}

		// Launch the request ordering routine
		go coreutils.StartRequestSequencer(agent.logicalDeviceID, agent.requestQueue, agent.requestComplete)
...
func (agent *LogicalDeviceAgent) stop(ctx context.Context) error {
	...
	agent.stopOnce.Do(func() {
		...
		if err := agent.waitForGreenLight(); err != nil {
...
func (agent *LogicalDeviceAgent) waitForGreenLight() error {
	if agent.stopped {
		return status.Errorf(codes.Aborted, "agent-already-stopped-%s", agent.logicalDeviceID)
	}
	ch := make(chan struct{})
	defer close(ch)
...
func (agent *LogicalDeviceAgent) yieldToNextRequest() {
	agent.requestComplete <- struct{}{}
}
...
func (agent *LogicalDeviceAgent) ListLogicalDeviceFlows() (*ofp.Flows, error) {
	...
	logicalDevice, err := agent.GetLogicalDevice()
	if err != nil {
		return nil, err
	}
