...
func newLogicalDeviceAgent(id string, deviceID string, ldeviceMgr *LogicalDeviceManager,
	...
	agent.requestQueue = coreutils.NewRequestQueue(maxOrderedLogicalDeviceRequestQueueSize)
...
func (agent *LogicalDeviceAgent) start(ctx context.Context, loadFromDB bool) error {
	...
	go coreutils.StartRequestSequencer(agent.logicalDeviceID, agent.requestQueue, agent.requestCompleteIndication)
...
func (agent *LogicalDeviceAgent) waitForGreenLight(ctx context.Context) error {
	if agent.stopped {
