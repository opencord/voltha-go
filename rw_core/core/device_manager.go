...
func (dMgr *DeviceManager) DeleteAllUNILogicalPorts(ctx context.Context, parentDevice *voltha.Device) error {
	log.Debugw("DeleteAllUNILogicalPorts", log.Fields{"parent-device-id": parentDevice.Id})
	if err := dMgr.logicalDeviceMgr.deleteAllUNILogicalPorts(ctx, parentDevice); err != nil {
		log.Warnw("delete-all-uni-logical-ports-error", log.Fields{"parent-device-id": parentDevice.Id})
		return err
	}
	return nil
}
