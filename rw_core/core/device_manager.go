...
func (dMgr *DeviceManager) stopManagingDevice(id string) error {
	...
	if dMgr.IsDeviceInCache(id) { // Proceed only if an agent is present for this device
		if root, _ := dMgr.IsRootDevice(id); root == true {
			...
			if err := dMgr.core.deviceOwnership.AbandonDevice(ldeviceId); err != nil {
				log.Warnw("failed-abandoning-device-ownership", log.Fields{"deviceId": id, "error": err})
				return err
			}
			// stop managing the child devices
			childDeviceIds := dMgr.getAllDeviceIdsWithDeviceParentId(id)
			for _, cId := range childDeviceIds {
				if err := dMgr.stopManagingDevice(cId); err != nil {
					log.Warnw("failed-stopping-managing-device", log.Fields{"deviceId": id, "error": err})
				}
			}
		}
		if agent := dMgr.getDeviceAgent(id); agent != nil {
			...
			if err := dMgr.core.deviceOwnership.AbandonDevice(id); err != nil {
				log.Warnw("failed-abandoning-device-ownership", log.Fields{"deviceId": id, "error": err})
				return err
			}
		}
...
func (dMgr *DeviceManager) RunPostDeviceDelete(cDevice *voltha.Device) error {
	...
	return dMgr.stopManagingDevice(cDevice.Id)
}
