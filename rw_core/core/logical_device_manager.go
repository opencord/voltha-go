...
type LogicalDeviceManager struct {
	...
	logicalDeviceLoadingInProgress sync.Map // Item in map defined as {logicalDeviceid, []channels}
}
...
func (ldMgr *LogicalDeviceManager) load(lDeviceId string) error {
	...
	if val, exist := ldMgr.logicalDeviceLoadingInProgress.Load(lDeviceId); !exist {
		if ldAgent, _ := ldMgr.logicalDeviceAgents.Load(lDeviceId); ldAgent == nil {
			ldMgr.logicalDeviceLoadingInProgress.Store(lDeviceId, []chan int{make(chan int, 1)})
			...
			ldMgr.logicalDevicesLoadingLock.Lock()
			if v, ok := ldMgr.logicalDeviceLoadingInProgress.Load(lDeviceId); ok {
				for _, ch := range v.([]chan int) {
					close(ch)
				}
				ldMgr.logicalDeviceLoadingInProgress.Delete(lDeviceId)
			}
			ldMgr.logicalDevicesLoadingLock.Unlock()
		} else {
			ldMgr.logicalDevicesLoadingLock.Unlock()
		...
		ldMgr.logicalDeviceLoadingInProgress.Store(lDeviceId, chnls)
		ldMgr.logicalDevicesLoadingLock.Unlock()
