...
func (nb *NBTest) startCore(inCompeteMode bool) {
	...
	nb.eventMgr = event.NewManager()
	nb.adapterMgr = adapter.NewAdapterManager(proxy, nb.coreInstanceID, nb.kClient)
	nb.deviceMgr, nb.logicalDeviceMgr = device.NewManagers(proxy, nb.adapterMgr, nb.eventMgr, nb.kmp, endpointMgr, cfg.CorePairTopic, nb.coreInstanceID, cfg.DefaultCoreTimeout)
...
func TestSuiteNbiApiHandler(t *testing.T) {
	...
	nbi := NewNBIHandler(nb.deviceMgr, nb.logicalDeviceMgr, nb.adapterMgr, nb.eventMgr)
