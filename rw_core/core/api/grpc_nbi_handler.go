...
type eventManager struct{ *event.Manager }

// NewNBIHandler creates API handler instance
func NewNBIHandler(deviceMgr *device.Manager, logicalDeviceMgr *device.LogicalManager, adapterMgr *adapter.Manager, eventMgr *event.Manager) *NBIHandler {
	return &NBIHandler{
		Manager:        deviceMgr,
		LogicalManager: logicalDeviceMgr,
		adapterManager: adapterManager{adapterMgr},
		eventManager:   eventManager{eventMgr},
	}
}
