...
func (rhp *AdapterRequestHandlerProxy) PortsStateUpdate(ctx context.Context, args []*ic.Argument) (*empty.Empty, error) {
	...
	if err := rhp.deviceMgr.UpdatePortsState(context.TODO(), deviceID.Id, voltha.Port_PortType(filter.Val), voltha.OperStatus_Types(operStatus.Val)); err != nil {
...
// GetDevicePorts returns a single port
func (rhp *AdapterRequestHandlerProxy) GetDevicePorts(ctx context.Context, args []*ic.Argument) (*voltha.Port, error) {
	if len(args) < 3 {
		logger.Warn(ctx, "invalid-number-of-args", log.Fields{"args": args})
		err := errors.New("invalid-number-of-args")
		return nil, err
	...
	logger.Debugw(ctx, "GetDevicePorts", log.Fields{"deviceID": deviceID.Id, "portNo": portNo.Val, "transactionID": transactionID.Val})
