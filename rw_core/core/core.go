 ...
 * more frequent interval. While NotLive, produce attempts that fail result
 * in no messages actually being sent out on the kafka bus. The final message
 * during a NotLive phase will succeed (transitioning back to live) and will
 * be sent on the bus.
 *
