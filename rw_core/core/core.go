...
func NewCore(id string, cf *config.RWCoreFlags, kvClient kvstore.Client, kafkaClient kafka.Client) *Core {
	...
	var err error
	core.clusterDataProxy, err = core.clusterDataRoot.CreateProxy(context.Background(), "/", false); if err != nil {
		log.Fatalf("error %v", err)
	}
	core.localDataProxy, err = core.localDataRoot.CreateProxy(context.Background(), "/", false); if err != nil {
		log.Fatalf("error %v", err)
	}
	return &core
}
