...
type Core struct {
	...
	exitChannelRead   chan int // exitChannel, read-only
	exitChannelWrite  chan int // exitChannel, write-only
...
func NewCore(ctx context.Context, id string, cf *config.RWCoreFlags, kvClient kvstore.Client, kafkaClient kafka.Client) *Core {
	var core Core
	core.instanceID = id
	core.exitChannelWrite = make(chan int, 1)
	core.exitChannelRead = core.exitChannelWrite
	core.config = cf
