...
func (agent *DeviceAgent) start(ctx context.Context, loadFromdB bool) error {
	...
	if loadFromdB {
		...
		if added, err := agent.clusterDataProxy.AddWithID(ctx, "/devices", agent.deviceId, agent.lastData, ""); err != nil {
			return err
		} else if added == nil {
...
func (agent *DeviceAgent) initPmConfigs(pmConfigs *voltha.PmConfigs) error {
	...
	if storeDevice, err := agent.getDeviceWithoutLock(); err != nil {
		...
		afterUpdate, err := agent.clusterDataProxy.Update(updateCtx, "/devices/"+agent.deviceId, cloned, false, ""); if err != nil {
			return err
