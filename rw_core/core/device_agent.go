...
func (agent *DeviceAgent) updateAdminState(ctx context.Context, adminState voltha.AdminState_Types) error {
	...
	return agent.updateDeviceStateInStoreWithoutLock(ctx, cloned, adminState, cloned.ConnectStatus, cloned.OperStatus)
}
...
func (agent *DeviceAgent) updateDeviceStateInStoreWithoutLock(
	...
	go func() {
		if err := agent.deviceMgr.processTransition(ctx, device, getDeviceStates(device)); err != nil {
