...
func (aMgr *Manager) addAdapter(ctx context.Context, adapter *voltha.Adapter, saveToDb bool) error {
	...
	if _, exist := aMgr.adapterAgents[adapter.Id]; !exist {
		...
		aMgr.adapterAgents[adapter.Id] = newAdapterAgent(clonedAdapter, aMgr.onAdapterRestart, aMgr.liveProbeInterval)
...
func (aMgr *Manager) RegisterAdapter(ctx context.Context, registration *ic.AdapterRegistration) (*empty.Empty, error) {
	...
	logger.Debugw(ctx, "RegisterAdapter", log.Fields{"adapter": adapter, "deviceTypes": deviceTypes.Items})
	...
	if adpt, _ := aMgr.getAdapter(ctx, adapter.Id); adpt != nil {
		// Adapter has restarted. Just log as restart handler will handle the restart flow
		logger.Warnw(ctx, "adapter-restarted", log.Fields{"adapter": adpt.Id})
