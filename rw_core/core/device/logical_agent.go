...
func (agent *LogicalAgent) setupNNILogicalPorts(ctx context.Context, deviceID string) error {
	...
	for _, port := range device.Ports {
		if port.Type == voltha.Port_ETHERNET_NNI {
			...
			agent.addEmptyFlowChunkToModel(ctx, port.PortNo)
		}
	}
	return err
}
...
func (agent *LogicalAgent) decomposeAndAdd(ctx context.Context, flow *ofp.OfpFlowStats, mod *ofp.OfpFlowMod) (bool, bool, error) {
	...
	if !ok {
		...
		flowsChunk = agent.addEmptyFlowChunkToModel(ctx, portClassifier)
	}
...
func (agent *LogicalAgent) addEmptyFlowChunkToModel(ctx context.Context, portNo uint32) *FlowsChunk {
