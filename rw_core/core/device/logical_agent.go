...
func (agent *LogicalAgent) updateFlowCountOfMeterStats(ctx context.Context, modCommand *ofp.OfpFlowMod, flow *ofp.OfpFlowStats) bool {
...
func (agent *LogicalAgent) flowAdd(ctx context.Context, mod *ofp.OfpFlowMod) error {
	...
	if changed && !updated {
		if dbupdated := agent.updateFlowCountOfMeterStats(ctx, mod, flow); !dbupdated {
	...
	if changed {
		...
		go func() {
			...
			if res := coreutils.WaitForNilOrErrorResponses(agent.defaultTimeout, respChannels...); res != nil {
				logger.Warnw("failure-to-add-flows", log.Fields{"errors": res, "logical-device-id": agent.logicalDeviceID, "timeout": agent.defaultTimeout})
...
func (agent *LogicalAgent) flowDeleteStrict(ctx context.Context, mod *ofp.OfpFlowMod) error {
	...
	idx := fu.FindFlows(flows, flow)
	if idx >= 0 {
		if changedMeter := agent.updateFlowCountOfMeterStats(ctx, mod, flows[idx]); !changedMeter {
