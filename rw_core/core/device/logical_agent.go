...
type LogicalAgent struct {
	...
	meters    map[uint32]*MeterChunk // meter_id to MeterChunk
	meterLock sync.RWMutex           // only used for adding a new entry to meterIDToMeterMap
	flows     map[uint64]*FlowsChunk // logical_port to Flows
	flowLock  sync.RWMutex           // only used for adding a new entry to logicalPortToFlowsMap
	groups    map[uint32]*GroupChunk // group_id to Group map
...
func newLogicalDeviceAgent(id string, sn string, deviceID string, ldeviceMgr *LogicalManager,
	...
	agent.flows = make(map[uint64]*FlowsChunk)
...
//updateLogicalDeviceFlowInStore updates flow in the store.
func (agent *LogicalAgent) updateLogicalDeviceFlowInStore(ctx context.Context, flow *ofp.OfpFlowStats) error {
...
//removeLogicalDeviceFlowInStore deletes the flow from store
func (agent *LogicalAgent) removeLogicalDeviceFlowInStore(ctx context.Context, flowID uint64) error {
	path := fmt.Sprintf("logical_flows/%s/%d", agent.logicalDeviceID, flowID)
	if err := agent.clusterDataProxy.Remove(ctx, path); err != nil {
		return errors.New("couldnt-deleted-flow-from-store")
	}
	return nil
}

//updateLogicalDeviceMeterInStore updates meter info in store
func (agent *LogicalAgent) updateLogicalDeviceMeterInStore(ctx context.Context, meter *ofp.OfpMeterEntry) error {
...
//removeLogicalDeviceMeterInStore deletes the meter from store
...
//updateLogicalDeviceFlowGroupInStore updates the flow groups in store
func (agent *LogicalAgent) updateLogicalDeviceFlowGroupInStore(ctx context.Context, groupEntry *ofp.OfpGroupEntry) error {
...
//removeLogicalDeviceFlowGroupInStore removes the flow groups in store
func (agent *LogicalAgent) removeLogicalDeviceFlowGroupInStore(ctx context.Context, groupID uint32) error {
...
func (agent *LogicalAgent) meterModify(ctx context.Context, meterMod *ofp.OfpMeterMod) error {
	...
	if ok {
		...
		if err := agent.updateLogicalDeviceMeterInStore(ctx, newMeter); err != nil {
...
func (agent *LogicalAgent) deleteFlowsOfMeter(ctx context.Context, meterID uint32) error {
	...
	agent.flowLock.RLock()
	flowsToRemove := make([]*ofp.OfpFlowStats, 0)
	for _, flowChunk := range agent.flows {
		if mID := fu.GetMeterIdFromFlow(flowChunk.flow); mID != 0 && mID == meterID {
			...
			flowsToRemove = append(flowsToRemove, flowChunk.flow)
		}
	}
	agent.flowLock.RUnlock()
	agent.flowLock.Lock()
	defer agent.flowLock.Unlock()
	for _, flw := range flowsToRemove {
		//update cache and db.
		delete(agent.flows, flw.Id)
		if err := agent.updateLogicalDeviceFlowInStore(ctx, flw); err != nil {
			return err
		}
	}
...
func (agent *LogicalAgent) updateFlowCountOfMeterStats(ctx context.Context, modCommand *ofp.OfpFlowMod, flow *ofp.OfpFlowStats, revertUpdate bool) bool {
	...
	//	Update model
	if err := agent.updateLogicalDeviceMeterInStore(ctx, meterChunk.meter); err != nil {
...
func (agent *LogicalAgent) flowAdd(ctx context.Context, mod *ofp.OfpFlowMod) error {
	...
	logger.Debugw("HashFlowStats-in-flowAdd", log.Fields{"FlowID": flow.Id, "TableId": flow.TableId, "Priority": flow.Priority, "Flags": flow.Flags, "Cookie": flow.Cookie, "Match": flow.Match.String()})
	if err != nil {
...
func (agent *LogicalAgent) decomposeAndAdd(ctx context.Context, flow *ofp.OfpFlowStats, mod *ofp.OfpFlowMod) (bool, bool, error) {
	...
	flowChunk, ok := agent.getFlowInCache(flow.Id) //not found
	if !ok {
		//if flow is not found in the map, create an entry in the map
		flowChunk = &FlowsChunk{
			flow: flow,
		}
		alreadyExist = false
	}
	...
	if changed {
		...
		agent.flowLock.Lock()
		//	Update model
		if err := agent.updateLogicalDeviceFlowInStore(ctx, flow); err != nil {
...
func (agent *LogicalAgent) revertAddedFlows(ctx context.Context, mod *ofp.OfpFlowMod, addedFlow *ofp.OfpFlowStats, replacedFlow *ofp.OfpFlowStats, deviceRules *fu.DeviceRules, metadata *voltha.FlowMetadata) error {
	...
	flowsChunk, ok := agent.getFlowInCache(addedFlow.Id) //not found
	if !ok {
	...
	agent.flowLock.Lock()
	if replacedFlow != nil {
		flowsChunk.flow = replacedFlow
		if err := agent.updateLogicalDeviceFlowInStore(ctx, replacedFlow); err != nil {
			logger.Errorw("db-flow-update-failed", log.Fields{"logicalDeviceId": agent.logicalDeviceID, "error": err})
			agent.flowLock.Unlock()
			return err
		}
	} else {
		delete(agent.flows, addedFlow.Id)
		if err := agent.removeLogicalDeviceFlowInStore(ctx, addedFlow.Id); err != nil {
			logger.Errorw("db-flow-update-failed", log.Fields{"logicalDeviceId": agent.logicalDeviceID, "error": err})
			agent.flowLock.Unlock()
			return err
		}
...
func (agent *LogicalAgent) flowDelete(ctx context.Context, mod *ofp.OfpFlowMod) error {
	...
	if len(toDelete) > 0 {
		...
		for _, flw := range toDelete {
			// Update model
			if err := agent.deleteFlow(ctx, flw.Id); err != nil {
...
func (agent *LogicalAgent) flowDeleteStrict(ctx context.Context, mod *ofp.OfpFlowMod) error {
	...
	flow, err := fu.FlowStatsEntryFromFlowModMessage(mod)
	logger.Debugw("HashFlowStats-in-flowDeleteStrict", log.Fields{"FlowID": flow.Id, "TableId": flow.TableId, "Priority": flow.Priority, "Flags": flow.Flags, "Cookie": flow.Cookie, "Match": flow.Match.String()})
	if err != nil {
		return err
	}
	logger.Debugw("flow-id-in-flowDeleteStrict", log.Fields{"flowID": flow.Id})
	flowChunk, ok := agent.getFlowInCache(flow.Id)
	//not found
	if !ok {
	...
	// Update the model
	if err := agent.deleteFlow(ctx, flow.Id); err != nil {
		return err
	}
	// Update the devices
	if partialRoute {
...
func (agent *LogicalAgent) groupDelete(ctx context.Context, groupMod *ofp.OfpGroupMod) error {
	...
	if groupID == uint32(ofp.OfpGroup_OFPG_ALL) {
		...
		if ok {
			...
			defer groupChunk.lock.Unlock()
			//remove from store
			if err := agent.removeLogicalDeviceFlowGroupInStore(ctx, groupID); err != nil {
				return err
			}
...
func (agent *LogicalAgent) deleteFlowsOfGroup(ctx context.Context, groupID uint32) ([]*ofp.OfpFlowStats, error) {
	...
	var flowsToRemove []*ofp.OfpFlowStats
	agent.flowLock.RLock()
	for _, flowChunk := range agent.flows {
	...
	for _, flw := range flowsToRemove {
		//update cache and db.
		delete(agent.flows, flw.Id)
		if err := agent.removeLogicalDeviceFlowInStore(ctx, flw.Id); err != nil {
			return nil, err
		}
	}
	return flowsToRemove, nil
}
...
func (agent *LogicalAgent) groupModify(ctx context.Context, groupMod *ofp.OfpGroupMod) error {
	...
	if groupsChanged {
		...
		//update KV
		if err := agent.updateLogicalDeviceFlowGroupInStore(ctx, groupEntry); err != nil {
...
func (agent *LogicalAgent) deleteFlow(ctx context.Context, flowID uint64) error {
	path := fmt.Sprintf("logical_flows/%s/%d", agent.logicalDeviceID, flowID)
	agent.flowLock.Lock()
	if err := agent.clusterDataProxy.Remove(ctx, path); err != nil {
		return err
...
//getFlowInCache returns the flow in Map
func (agent *LogicalAgent) getFlowInCache(flowID uint64) (*FlowsChunk, bool) {
	agent.flowLock.RLock()
	flowsChunk, ok := agent.flows[flowID]
	agent.flowLock.RUnlock()
