...
// LoadFlows queries existing flows from the kv,
// and should only be called once when first created.
func (loader *Loader) LoadFlows(ctx context.Context) {
	loader.lock.Lock()
...
// LockOrCreateFlow locks this flow if it exists, or creates a new flow if it does not.
// In the case of flow creation, the provided "flow" must not be modified afterwards.
func (loader *Loader) LockOrCreateFlow(ctx context.Context, flow *ofp.OfpFlowStats) (*Handle, bool, error) {
	// try to use read lock instead of full lock if possible
	if handle, have := loader.LockFlow(flow.Id); have {
	...
	if entry.deleted {
		entry.lock.Unlock()
		return loader.LockOrCreateFlow(ctx, flow)
	}
	return &Handle{loader: loader, chunk: entry}, false, nil
}

// LockFlow acquires a lock on the flow, and returns a handle in order to safely modify the flow.
// If not found, false is returned instead.
// TODO: consider accepting a ctx and aborting the lock attempt on cancellation
func (loader *Loader) LockFlow(id uint64) (*Handle, bool) {
	loader.lock.RLock()
...
// GetReadOnly returns an *ofp.OfpFlowStats which MUST NOT be modified externally, but is safe to keep indefinitely
...
// ListFlows returns a snapshot of all the managed flow IDs
// TODO: iterating through flows safely is expensive now, since all flows are stored & locked separately
func (loader *Loader) ListFlows() map[uint64]struct{} {
	loader.lock.RLock()
