...
type Loader struct {
	...
	devicePorts map[string]map[uint32]struct{}
...
func NewLoader(dbProxy *model.Proxy) *Loader {
	return &Loader{
		...
		devicePorts: make(map[string]map[uint32]struct{}),
		dbProxy:     dbProxy,
	}
}
...
func (loader *Loader) Load(ctx context.Context) {
	...
	for _, port := range ports {
		loader.addUnsafe(port.DeviceId, port.OfpPort.PortNo, &chunk{port: port})
	}
}
...
func (loader *Loader) LockOrCreate(ctx context.Context, port *voltha.LogicalPort) (*Handle, bool, error) {
	...
	if !have {
		...
		loader.addUnsafe(port.DeviceId, port.OfpPort.PortNo, entry)
		entry.lock.Lock()
		loader.lock.Unlock()
		...
		if err := loader.dbProxy.Set(ctx, fmt.Sprint(port.OfpPort.PortNo), port); err != nil {
			// revert the map
			loader.lock.Lock()
			loader.deletedUnsafe(port.DeviceId, port.OfpPort.PortNo)
			loader.lock.Unlock()
...
func (loader *Loader) Lock(portNo uint32) (*Handle, bool) {
	loader.lock.RLock()
	entry, have := loader.ports[portNo]
	loader.lock.RUnlock()
...
func (h *Handle) Update(ctx context.Context, port *voltha.LogicalPort) error {
	if err := h.loader.dbProxy.Set(ctx, fmt.Sprint(port.OfpPort.PortNo), port); err != nil {
		return status.Errorf(codes.Internal, "failed-update-port-%s-%d: %s", port.DeviceId, port.OfpPort.PortNo, err)
	}
	h.chunk.port = port
	return nil
}
...
func (h *Handle) Delete(ctx context.Context) error {
	port := h.chunk.port
	if err := h.loader.dbProxy.Remove(ctx, fmt.Sprint(port.OfpPort.PortNo)); err != nil {
		return fmt.Errorf("couldnt-delete-port-from-store-%s-%d", port.DeviceId, port.OfpPort.PortNo)
	}
	h.chunk.deleted = true

	h.loader.lock.Lock()
	h.loader.deletedUnsafe(port.DeviceId, port.OfpPort.PortNo)
...
func (loader *Loader) ListForDevice(deviceID string) map[uint32]struct{} {
	...
	defer loader.lock.RUnlock()
	// copy the IDs so caller can safely iterate
	devicePorts := loader.devicePorts[deviceID]
...
func (loader *Loader) addUnsafe(deviceID string, portNo uint32, entry *chunk) {
	loader.ports[portNo] = entry
	if devicePorts, have := loader.devicePorts[deviceID]; have {
		devicePorts[portNo] = struct{}{}
	} else {
		loader.devicePorts[deviceID] = map[uint32]struct{}{portNo: {}}
	}
}

func (loader *Loader) deletedUnsafe(deviceID string, portNo uint32) {
	delete(loader.ports, portNo)
	if devicePorts, have := loader.devicePorts[deviceID]; have {
		delete(devicePorts, portNo)
		if len(devicePorts) == 0 {
			delete(loader.devicePorts, deviceID)
		}
	}
}
