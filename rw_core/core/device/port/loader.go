...
func (loader *Loader) Load(ctx context.Context) {
	...
	for _, port := range ports {
		loader.ports[port.Id] = &chunk{port: port}
	}
}
...
func (loader *Loader) LockOrCreate(ctx context.Context, port *voltha.LogicalPort) (*Handle, bool, error) {
	...
	if handle, have := loader.Lock(port.Id); have {
		return handle, false, nil
	}

	loader.lock.Lock()
	entry, have := loader.ports[port.Id]
	if !have {
		entry := &chunk{port: port}
		loader.ports[port.Id] = entry
		entry.lock.Lock()
		loader.lock.Unlock()

		if err := loader.dbProxy.Set(ctx, fmt.Sprint(port.Id), port); err != nil {
			logger.Errorw("failed-adding-port-to-db", log.Fields{"portID": port.Id, "err": err})

			// revert the map
			loader.lock.Lock()
			delete(loader.ports, port.Id)
			loader.lock.Unlock()
...
func (h *Handle) Update(ctx context.Context, port *voltha.LogicalPort) error {
	if err := h.loader.dbProxy.Set(ctx, fmt.Sprint(port.Id), port); err != nil {
		return status.Errorf(codes.Internal, "failed-update-port-%v: %s", port.Id, err)
	}
	h.chunk.port = port
	return nil
}
...
func (h *Handle) Delete(ctx context.Context) error {
	if err := h.loader.dbProxy.Remove(ctx, fmt.Sprint(h.chunk.port.Id)); err != nil {
		return fmt.Errorf("couldnt-delete-port-from-store-%v", h.chunk.port.Id)
	}
	h.chunk.deleted = true

	h.loader.lock.Lock()
	delete(h.loader.ports, h.chunk.port.Id)
...
// List returns a snapshot of all the managed port IDs
...
func (loader *Loader) List() map[string]struct{} {
	loader.lock.RLock()
	defer loader.lock.RUnlock()
	// copy the IDs so caller can safely iterate
	ret := make(map[string]struct{}, len(loader.ports))
