...
type Loader struct {
	...
	lock  sync.RWMutex
	ports map[string]map[uint32]*chunk
...
func NewLoader(dbProxy *model.Proxy) *Loader {
	return &Loader{
		ports:   make(map[string]map[uint32]*chunk),
		dbProxy: dbProxy,
	}
}
...
func (loader *Loader) Load(ctx context.Context) {
	...
	for _, port := range ports {
		loader.addUnsafe(port.DeviceId, port.DevicePortNo, &chunk{port: port})
	}
}
...
func (loader *Loader) LockOrCreate(ctx context.Context, port *voltha.LogicalPort) (*Handle, bool, error) {
	...
	if handle, have := loader.Lock(port.DeviceId, port.DevicePortNo); have {
		return handle, false, nil
	}

	loader.lock.Lock()
	entry, have := loader.ports[port.DeviceId][port.DevicePortNo]
	if !have {
		entry := &chunk{port: port}
		loader.addUnsafe(port.DeviceId, port.DevicePortNo, entry)
		entry.lock.Lock()
		loader.lock.Unlock()

		if err := loader.dbProxy.Set(ctx, fmt.Sprint(port.DeviceId, "/", port.DevicePortNo), port); err != nil {
			// revert the map
			loader.lock.Lock()
			loader.deletedUnsafe(port.DeviceId, port.DevicePortNo)
			loader.lock.Unlock()
...
func (loader *Loader) Lock(deviceID string, portNo uint32) (*Handle, bool) {
	loader.lock.RLock()
	entry, have := loader.ports[deviceID][portNo]
	loader.lock.RUnlock()
	...
	if entry.deleted {
		entry.lock.Unlock()
		return loader.Lock(deviceID, portNo)
	}
...
func (h *Handle) Update(ctx context.Context, port *voltha.LogicalPort) error {
	if err := h.loader.dbProxy.Set(ctx, fmt.Sprint(port.DeviceId, "/", port.DevicePortNo), port); err != nil {
		return status.Errorf(codes.Internal, "failed-update-port-%s-%d: %s", port.DeviceId, port.DevicePortNo, err)
	}
	h.chunk.port = port
	return nil
}
...
func (h *Handle) Delete(ctx context.Context) error {
	port := h.chunk.port
	if err := h.loader.dbProxy.Remove(ctx, fmt.Sprint(port.DeviceId, "/", port.DevicePortNo)); err != nil {
		return fmt.Errorf("couldnt-delete-port-from-store-%s-%d", port.DeviceId, port.DevicePortNo)
	}
	h.chunk.deleted = true

	h.loader.lock.Lock()
	h.loader.deletedUnsafe(port.DeviceId, port.DevicePortNo)
...
// List returns a snapshot of all the managed port identifiers
...
func (loader *Loader) List() map[ID]struct{} {
	loader.lock.RLock()
	defer loader.lock.RUnlock()
	// copy the IDs so caller can safely iterate
	length := 0
	for _, devicePorts := range loader.ports {
		length += len(devicePorts)
	}
	ret := make(map[ID]struct{}, length)
	for deviceID, ports := range loader.ports {
		for portNo := range ports {
			ret[ID{deviceID, portNo}] = struct{}{}
		}
	}
	return ret
}

// ListForDevice lists ports belonging to the specified device
func (loader *Loader) ListForDevice(deviceID string) map[uint32]struct{} {
	loader.lock.RLock()
	defer loader.lock.RUnlock()
	// copy the IDs so caller can safely iterate
	devicePorts := loader.ports[deviceID]
...
func (loader *Loader) addUnsafe(deviceID string, portNo uint32, entry *chunk) {
	if devicePorts, have := loader.ports[deviceID]; have {
		devicePorts[portNo] = entry
	} else {
		loader.ports[deviceID] = map[uint32]*chunk{portNo: entry}
	}
}

func (loader *Loader) deletedUnsafe(deviceID string, portNo uint32) {
	if devicePorts, have := loader.ports[deviceID]; have {
		delete(devicePorts, portNo)
		if len(devicePorts) == 0 {
			delete(loader.ports, deviceID)
		}
	}
}
