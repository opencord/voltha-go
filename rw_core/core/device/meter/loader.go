...
// LoadMeters queries existing meters from the kv,
// and should only be called once when first created.
func (loader *Loader) LoadMeters(ctx context.Context) {
	loader.lock.Lock()
...
// LockOrCreateMeter locks this meter if it exists, or creates a new meter if it does not.
// In the case of meter creation, the provided "meter" must not be modified afterwards.
func (loader *Loader) LockOrCreateMeter(ctx context.Context, meter *ofp.OfpMeterEntry) (*Handle, bool, error) {
	// try to use read lock instead of full lock if possible
	if handle, have := loader.LockMeter(meter.Config.MeterId); have {
	...
	if entry.deleted {
		entry.lock.Unlock()
		return loader.LockOrCreateMeter(ctx, meter)
	}
	return &Handle{loader: loader, chunk: entry}, false, nil
}

// LockMeter acquires a lock on the meter, and returns a handle in order to safely modify the meter.
// If not found, false is returned instead.
// TODO: consider accepting a ctx and aborting the lock attempt on cancellation
func (loader *Loader) LockMeter(id uint32) (*Handle, bool) {
	loader.lock.RLock()
...
// GetReadOnly returns an *ofp.OfpMeterEntry which MUST NOT be modified externally, but is safe to keep indefinitely
...
// ListMeters returns a snapshot of all the managed meter IDs
// TODO: iterating through meters safely is expensive now, since all meters are stored & locked separately
func (loader *Loader) ListMeters() map[uint32]struct{} {
	loader.lock.RLock()
