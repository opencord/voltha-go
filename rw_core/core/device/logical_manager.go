...
loop:
	for {
		...
		if err != nil {
			...
			go ldMgr.GetAndSendRPCEvent(ctx, "StreamPacketsOut", packet.Id, err.Error(), nil,
...
func (ldMgr *LogicalManager) GetAndSendRPCEvent(ctx context.Context, rpc, resourceID, desc string, context map[string]string,
	...
	ldMgr.Manager.RPCEventManager.GetAndSendRPCEvent(ctx, rpc, resourceID, desc, context, id,
