...
func (agent *Agent) newDeviceUpdate(ctx context.Context, operation string) *voltha.DeviceUpdate {
	...
	if span := opentracing.SpanFromContext(ctx); span != nil {
		if jSpan, ok := span.(*jtracing.Span); ok {
			...
			logger.Warnf(ctx, "isRootSpan: %v", isRootSpan)
			if isRootSpan{
				requestedBy = span.BaggageItem("fromTopic")
			}else{
				tagMap := jSpan.Tags()
				if rb := tagMap["fromTopic"]; rb != nil{
					logger.Warnf(ctx, "Tags: fromTopic: %v", rb )
					requestedBy = fmt.Sprintf("%v", rb)
				}
			}*/
		}
	}
