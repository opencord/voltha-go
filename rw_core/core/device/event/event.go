	...
	for {
		...
		case packet := <-q.packetInQueue:
			...
			if err := packetsIn.Send(&packet); err != nil {
				...
				_ = q.RPCEventManager.GetAndSendRPCEvent(ctx, "ReceivePacketsIn", "", err.Error(),
	...
	for {
		...
		case event := <-q.changeEventQueue:
			...
			if err := changeEvents.Send(&event); err != nil {
				...
				_ = q.RPCEventManager.GetAndSendRPCEvent(ctx, "ReceiveChangeEvents", "", err.Error(),
...
func (q *RPCEventManager) NewRPCEvent(ctx context.Context, rpc, resourceID, desc string, context map[string]string) *voltha.RPCEvent {
...
func (q *RPCEventManager) SendRPCEvent(ctx context.Context, id string, rpcEvent *voltha.RPCEvent, category voltha.EventCategory_Types, subCategory *voltha.EventSubCategory_Types, raisedTs int64) error {
	...
	go q.eventProxy.SendRPCEvent(ctx, id, rpcEvent, category, subCategory, raisedTs)
	return nil
}

func (q *RPCEventManager) GetAndSendRPCEvent(ctx context.Context, rpc, resourceID, desc string, context map[string]string,
	id string, category voltha.EventCategory_Types, subCategory *voltha.EventSubCategory_Types, raisedTs int64) error {
	rpcEvent := q.NewRPCEvent(ctx, rpc, resourceID, desc, context)
	//TODO Instead of directly sending to the kafka bus, queue the message and send it asynchronously
	go q.eventProxy.SendRPCEvent(ctx, id, rpcEvent, category, subCategory, raisedTs)
	return nil
}
