...
type Manager struct {
	...
	eventQueue        chan openflow_13.OFEvent
...
func NewManager() *Manager {
	return &Manager{
		...
		eventQueue:        make(chan openflow_13.OFEvent, 100),
...
func (q *Manager) flushFailedPackets(ctx context.Context, tracker *callTracker) error {
	if tracker.failedPacket != nil {
		...
		case openflow_13.OFEvent:
			logger.Debug(ctx, "Enqueueing last failed event")
			q.eventQueue <- tracker.failedPacket.(openflow_13.OFEvent)
		}
	}
	return nil
}
...
func (q *Manager) SendEvent(ctx context.Context, deviceID string, reason openflow_13.OfpPortReason, desc *openflow_13.OfpPort) {
	...
	q.eventQueue <- openflow_13.OFEvent{
		Id: deviceID,
		Event: &openflow_13.OFEvent_PortStatus{
...
// ReceiveEvents receives change in events
func (q *Manager) ReceiveEvents(_ *empty.Empty, events voltha.VolthaService_ReceiveEventsServer) error {
	ctx := context.Background()
	var streamingTracker = q.getStreamingTracker(ctx, "ReceiveEvents", q.eventQueueDone)
	logger.Debugw(ctx, "ReceiveEvents-request", log.Fields{"events": events})
...
loop:
	for {
		...
		case <-q.eventQueueDone:
			logger.Debug(ctx, "Another ReceiveEvents already running. Bailing out ...")
			break loop
		}
	}
...
func (q *Manager) GetEventsQueueForTest() <-chan openflow_13.OFEvent {
	return q.eventQueue
}
