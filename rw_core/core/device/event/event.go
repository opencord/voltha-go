...
type Manager struct {
	...
	eventQueue        chan openflow_13.Event
...
func NewManager() *Manager {
	return &Manager{
		...
		eventQueue:        make(chan openflow_13.Event, 100),
...
func (q *Manager) flushFailedPackets(ctx context.Context, tracker *callTracker) error {
	if tracker.failedPacket != nil {
		...
		case openflow_13.Event:
			logger.Debug(ctx, "Enqueueing last failed event")
			q.eventQueue <- tracker.failedPacket.(openflow_13.Event)
		}
	}
	return nil
}
...
func (q *Manager) SendEvent(ctx context.Context, deviceID string, reason openflow_13.OfpPortReason, desc *openflow_13.OfpPort) {
	...
	q.eventQueue <- openflow_13.Event{
		Id: deviceID,
		Event: &openflow_13.Event_PortStatus{
...
func (q *Manager) GetEventsQueueForTest() <-chan openflow_13.Event {
	return q.eventQueue
}
