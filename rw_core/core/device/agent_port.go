...
func (agent *Agent) deleteAllPorts(ctx context.Context) error {
	...
	if device.AdminState != voltha.AdminState_DISABLED && !agent.isProcessingDeletingTransientState() {
		err := status.Error(codes.FailedPrecondition, fmt.Sprintf("invalid-state-%v-or-delete-in-progress",
			device.AdminState))
