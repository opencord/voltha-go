		...
		for _, deviceID := range devicesInFlows {
			if err := agent.deviceMgr.canAdapterRequestProceed(ctx, deviceID); err != nil {
				...
				if status.Code(err) != codes.NotFound {
					...
					logger.Debugw(ctx, "flow-delete-from-nil-device-ignored", log.Fields{"deviceID": deviceID})
					if deviceRules != nil {
	...
	for _, deviceID := range devicesInFlows {
		if err := agent.deviceMgr.canAdapterRequestProceed(ctx, deviceID); err != nil {
			...
			if status.Code(err) != codes.NotFound {
				...
				logger.Debugw(ctx, "flow-delete-strict-from-nil-device-ignored", log.Fields{"deviceID": deviceID})
				if deviceRules != nil {
					deviceRules.RemoveRule(deviceID)
				}
			}
