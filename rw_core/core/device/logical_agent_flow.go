	...
	if changed {
		...
		go func() {
			...
			if res := coreutils.WaitForNilOrErrorResponses(agent.defaultTimeout, respChannels...); res != nil {
				...
				rpce := agent.ldeviceMgr.Manager.RpcEventManager.NewRPCEvent(ctx, "UpdateLogicalDeviceFlowTable", agent.logicalDeviceID, "failed-to-add-flow")
	...
	if len(toDelete) > 0 {
		...
		go func() {
			...
			if res := coreutils.WaitForNilOrErrorResponses(agent.defaultTimeout, respChnls...); res != nil {
				...
				rpce := agent.ldeviceMgr.Manager.RpcEventManager.NewRPCEvent(ctx, "UpdateLogicalDeviceFlowTable", agent.logicalDeviceID, "failed-to-update-device-flows")
				context := make(map[string]string)
				context["deviceRules"] = deviceRules.String()
				rpce.Context = context
				// Create context and send extra information as part of it.
...
func (agent *LogicalAgent) flowDeleteStrict(ctx context.Context, mod *ofp.OfpFlowMod) error {
	...
	go func() {
		if res := coreutils.WaitForNilOrErrorResponses(agent.defaultTimeout, respChnls...); res != nil {
			...
			rpce := agent.ldeviceMgr.Manager.RpcEventManager.NewRPCEvent(ctx, "UpdateLogicalDeviceFlowTable", agent.logicalDeviceID, "failed-to-delete-device-flows")
			context := make(map[string]string)
			context["flow-id"] = string(flow.Id)
			context["deviceRules"] = deviceRules.String()
			rpce.Context = context
