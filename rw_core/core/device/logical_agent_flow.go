...
func (agent *LogicalAgent) ListLogicalDeviceFlows(ctx context.Context) (*ofp.Flows, error) {
	...
	flowIDs := agent.flowLoader.ListFlows()
	ret := make([]*ofp.OfpFlowStats, 0, len(flowIDs))
	for flowID := range flowIDs {
		if flowHandle, have := agent.flowLoader.LockFlow(flowID); have {
			ret = append(ret, flowHandle.GetReadOnly())
			flowHandle.Unlock()
		}
	}
...
func (agent *LogicalAgent) decomposeAndAdd(ctx context.Context, flow *ofp.OfpFlowStats, mod *ofp.OfpFlowMod) (bool, bool, error) {
	...
	flowHandle, created, err := agent.flowLoader.LockOrCreateFlow(ctx, flow)
	if err != nil {
	...
	if changed {
		...
		groupIDs := agent.groupLoader.ListGroups()
		groups := make(map[uint32]*ofp.OfpGroupEntry, len(groupIDs))
		for groupID := range groupIDs {
			if groupHandle, have := agent.groupLoader.LockGroup(groupID); have {
...
func (agent *LogicalAgent) revertAddedFlows(ctx context.Context, mod *ofp.OfpFlowMod, addedFlow *ofp.OfpFlowStats, replacedFlow *ofp.OfpFlowStats, deviceRules *fu.DeviceRules, metadata *voltha.FlowMetadata) error {
	...
	flowHandle, have := agent.flowLoader.LockFlow(addedFlow.Id)
	if !have {
...
func (agent *LogicalAgent) flowDelete(ctx context.Context, mod *ofp.OfpFlowMod) error {
	...
	if handle, have := agent.flowLoader.LockFlow(fs.Id); have {
		toDelete[fs.Id] = handle.GetReadOnly()
		handle.Unlock()
	}

	// search through all the flows
	for flowID := range agent.flowLoader.ListFlows() {
		if flowHandle, have := agent.flowLoader.LockFlow(flowID); have {
	...
	if len(toDelete) > 0 {
		...
		for _, flow := range toDelete {
			if flowHandle, have := agent.flowLoader.LockFlow(flow.Id); have {
		...
		for groupID := range agent.groupLoader.ListGroups() {
			if groupHandle, have := agent.groupLoader.LockGroup(groupID); have {
				groups[groupID] = groupHandle.GetReadOnly()
				groupHandle.Unlock()
			}
		...
		// Update the devices
		if partialRoute {
			respChnls = agent.deleteFlowsFromParentDevice(ctx, toDelete, toMetadata(metersConfig))
		} else {
			respChnls = agent.deleteFlowsAndGroupsFromDevices(ctx, deviceRules, toMetadata(metersConfig))
		}
...
func (agent *LogicalAgent) flowDeleteStrict(ctx context.Context, mod *ofp.OfpFlowMod) error {
	...
	flowHandle, have := agent.flowLoader.LockFlow(flow.Id)
	if !have {
	...
	// Update the devices
	if partialRoute {
		respChnls = agent.deleteFlowsFromParentDevice(ctx, flowsToDelete, toMetadata(flowMetadata))
	} else {
		respChnls = agent.deleteFlowsAndGroupsFromDevices(ctx, deviceRules, toMetadata(flowMetadata))
	}
