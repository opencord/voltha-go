...
func (agent *Agent) updateDeviceAndReleaseLock(ctx context.Context, device *voltha.Device) error {
	...
	if device.Root && device.OperStatus == common.OperStatus_REBOOTED {
		if err := agent.updateTransientState(ctx, core.DeviceTransientState_OLT_REBOOT_IN_PROGRESS); err != nil {
			return err
		}
		deviceTransientState = core.DeviceTransientState_OLT_REBOOT_IN_PROGRESS
	}
...
func (agent *Agent) abortAllProcessing(ctx context.Context) error {
	...
	case core.DeviceTransientState_OLT_REBOOT_IN_PROGRESS:
		// do not change state
		return nil
	default:
		updatedState = core.DeviceTransientState_NONE
...
retry:
	for {
		...
		if state != core.DeviceTransientState_OLT_REBOOT_IN_PROGRESS {
			if state != core.DeviceTransientState_RECONCILE_IN_PROGRESS {
				//set transient state to RECONCILE IN PROGRESS
				if err = agent.CheckAndUpdateTransientState(ctx); err != nil {
					logger.Errorw(ctx, "setting-transient-state-failed", log.Fields{"error": err})
					agent.stopReconcilingMutex.Lock()
					if agent.stopReconciling != nil {
		...
		if err != nil || state == core.DeviceTransientState_OLT_REBOOT_IN_PROGRESS {
			agent.logDeviceUpdate(ctx, nil, nil, requestStatus, err, desc)
			select {
			case <-backoffTimer.C:
