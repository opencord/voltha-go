...
func (agent *DeviceAgent) getValue(ctx context.Context, pdevice *voltha.Device, cdevice *voltha.Device, valueparam *voltha.ValueSpecifier ) (*voltha.ReturnValues, error) {
	...
	//send request to adapter
        subCtx, cancel := context.WithTimeout(context.Background(), agent.defaultTimeout)
	ch, err := agent.adapterProxy.getValue(ctx, pdevice, cdevice, valueparam.DeviceID, valueparam.Value)
	go agent.waitForAdapterResponse(subCtx, cancel, "getValue", ch, agent.onSuccess, agent.onFailure)
	if err != nil {
		return nil, err
	}
        // Wait for the adapter response
