...
func (agent *Agent) onSuccess(ctx context.Context, rpc string, response interface{}, reqArgs ...interface{}) {
	...
	rpcID := fmt.Sprintf("RPC_SUCCESS") // there is no standard format to define this, seems like it is left to developer's imagination.

	logger.Debugw(ctx, "response-successful", log.Fields{"rpc": rpc, "device-id": agent.deviceID})
	// One success response for "updatePmConfigs" RPC we need to commit the voltha.Device object to DB.
