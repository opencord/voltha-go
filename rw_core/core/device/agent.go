...
func (agent *Agent) start(ctx context.Context, deviceToCreate *voltha.Device) (*voltha.Device, error) {
	...
	if deviceToCreate == nil {
		...
		_ = agent.deviceMgr.BusEventManager.SendDeviceStateChangeEvent(ctx, device.OperStatus, device.ConnectStatus, voltha.AdminState_UNKNOWN, device, time.Now().UnixNano())
...
func (agent *Agent) stop(ctx context.Context) error {
	...
	_ = agent.deviceMgr.BusEventManager.SendDeviceStateChangeEvent(ctx, prevOperStatus, device.ConnectStatus, device.AdminState, device, time.Now().UnixNano())
...
func (agent *Agent) updateDeviceAndReleaseLock(ctx context.Context, device *voltha.Device) error {
	...
	if prevDevice.OperStatus != device.OperStatus || prevDevice.ConnectStatus != device.ConnectStatus || prevDevice.AdminState != device.AdminState {
		_ = agent.deviceMgr.BusEventManager.SendDeviceStateChangeEvent(ctx, prevDevice.OperStatus, prevDevice.ConnectStatus, prevDevice.AdminState, device, time.Now().UnixNano())
	}
