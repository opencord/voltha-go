...
func (agent *Agent) enableOnuDevice(ctx context.Context, adapterEndpoint string) error {
	...
	newDevice, err := agent.checkEnableEligibility()
	if err != nil {
		agent.requestQueue.RequestComplete()
		return err
	}
...
func (agent *Agent) enableOnuSerialNumber(ctx context.Context, device *voltha.OnuSerialNumberOnOLTPon, onuAgent *Agent) error {
	var err error
	var desc string
	...
	if onuAgent != nil {
		...
		cloned, err = onuAgent.checkEnableEligibility()
		if err != nil {
...
func (agent *Agent) checkEnableEligibility() (*voltha.Device, error) {
	device := agent.getDeviceReadOnlyWithoutLock()
