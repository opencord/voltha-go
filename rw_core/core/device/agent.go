...
func (agent *Agent) addFlowsToAdapter(ctx context.Context, newFlows []*ofp.OfpFlowStats, flowMetadata *voltha.FlowMetadata) (coreutils.Response, error) {
	...
	if !dType.AcceptsAddRemoveFlowUpdates {
		...
		rpcResponse, err := agent.adapterProxy.UpdateFlowsBulk(subCtx, device, &voltha.Flows{Items: updatedAllFlows.Items}, &voltha.FlowGroups{Items: []*ofp.OfpGroupEntry{}}, flowMetadata)
		if err != nil {
			cancel()
...
func (agent *Agent) addGroupsToAdapter(ctx context.Context, newGroups []*ofp.OfpGroupEntry, flowMetadata *voltha.FlowMetadata) (coreutils.Response, error) {
	...
	if !dType.AcceptsAddRemoveFlowUpdates {
		...
		rpcResponse, err := agent.adapterProxy.UpdateFlowsBulk(subCtx, device, &voltha.Flows{Items: []*ofp.OfpFlowStats{}}, &voltha.FlowGroups{Items: updatedAllGroups.Items}, flowMetadata)
		if err != nil {
			cancel()
			return coreutils.DoneResponse(), err
		}
