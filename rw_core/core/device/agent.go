...
func (agent *Agent) disableChildDevice(ctx context.Context, adapterEndpoint string) error {
	var err error
	var desc string
	var prevAdminState, currAdminState common.AdminState_Types
	requestStatus := &common.OperationResp{Code: common.OperationResp_OPERATION_FAILURE}
	...
	cloned, desc, err := agent.checkDisableEligibility(ctx)
	if err != nil {
		agent.requestQueue.RequestComplete()
		return err
	}
	...
	go func() {
		if _, callErr := client.DisableChildDevice(subCtx, cloned); callErr == nil {
...
func (agent *Agent) enableChildDevice(ctx context.Context, adapterEndpoint string) error {
	var err error
	var desc string
	var prevAdminState, currAdminState common.AdminState_Types
	requestStatus := &common.OperationResp{Code: common.OperationResp_OPERATION_FAILURE}
	...
	newDevice, err := agent.checkEnableEligibility(ctx)
	if err != nil {
		agent.requestQueue.RequestComplete()
		return err
	}
	...
	go func() {
		_, callErr := client.EnableChildDevice(subCtx, newDevice)
...
func (agent *Agent) disableChildSerialNumber(ctx context.Context, device *voltha.OnuSerialNumberOfOLTPon, adapterEndpoint string) error {
	var err error
	var desc string
	var prevAdminState, currAdminState common.AdminState_Types
	requestStatus := &common.OperationResp{Code: common.OperationResp_OPERATION_FAILURE}
	...
	go func() {
		_, callErr := client.DisableChildSerialNumber(subCtx, device)
		if callErr == nil {
...
func (agent *Agent) enableChildSerialNumber(ctx context.Context, device *voltha.OnuSerialNumberOfOLTPon, onuAgent *Agent) error {
	var err error
	var desc string
	...
	// Use onuAgent for device state updates, if device is there in core
	if onuAgent != nil {
		defer func() { onuAgent.logDeviceUpdate(ctx, &prevAdminState, &currAdminState, requestStatus, err, desc) }()

		if err = onuAgent.requestQueue.WaitForGreenLight(ctx); err != nil {
			return err
		}

		cloned, err = onuAgent.checkEnableEligibility(ctx)
		if err != nil {
			onuAgent.requestQueue.RequestComplete()
			return err
		}
	...
	go func() {
		_, callErr := client.EnableChildSerialNumber(subCtx, device)
		if callErr == nil {
	...
	if onuAgent != nil && cloned != nil {
		if updateErr := onuAgent.updateDeviceAndReleaseLock(subCtx, cloned); updateErr != nil {
			return fmt.Errorf("update-device-failed: %w", updateErr)
		}
		currAdminState = cloned.AdminState
	}
...
func (agent *Agent) checkDisableEligibility(ctx context.Context) (*voltha.Device, string, error) {
	cloned := agent.cloneDeviceWithoutLock()
...
func (agent *Agent) checkEnableEligibility(ctx context.Context) (*voltha.Device, error) {
	device := agent.getDeviceReadOnlyWithoutLock()
