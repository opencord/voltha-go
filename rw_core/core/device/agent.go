...
// onForceDeleteSuccess is a common callback for scenarios where we receive a nil response following a delete request
// to an adapter.
func (agent *Agent) onForceDeleteResponse(ctx context.Context, rpc string, response interface{}, reqArgs ...interface{}) {
	logger.Debugw(ctx, "force-delete-response-received", log.Fields{"rpc": rpc, "device-id": agent.deviceID})
	if err := agent.requestQueue.WaitForGreenLight(ctx); err != nil {
		logger.Errorw(ctx, "force-delete-device-failure", log.Fields{"device-id": agent.deviceID, "error": err, "args": reqArgs})
	}
	previousDeviceTransientState := agent.getTransientState()
	newDevice := agent.cloneDeviceWithoutLock()
	if err := agent.updateDeviceWithTransientStateAndReleaseLock(ctx, newDevice,
		...
		logger.Errorw(ctx, "delete-device-failure", log.Fields{"device-id": agent.deviceID, "error": err, "args": reqArgs})
		//Only updating of transient state is required in error, no transition.
		if err := agent.updateTransientState(ctx, voltha.DeviceTransientState_DELETE_FAILED); err != nil {
			logger.Errorw(ctx, "failed-to-update-transient-state-as-force-delete-failed",log.Fields{"device-id": agent.deviceID})
		}
	}
}
...
func (agent *Agent) deleteDeviceForce(ctx context.Context) error {
	...
	if previousAdminState == ic.AdminState_PREPROVISIONED {
		// Change the state to FORCE DELETING directly as adapters have no info of the device.
		currentDeviceTransientState = voltha.DeviceTransientState_FORCE_DELETING
	}
	...
	if previousAdminState != ic.AdminState_PREPROVISIONED {
		...
		go agent.waitForAdapterResponse(subCtx, cancel, "deleteDeviceForce", ch, agent.onForceDeleteResponse,
			agent.onForceDeleteResponse)
	}
	return nil
}
