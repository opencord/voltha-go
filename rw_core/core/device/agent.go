...
func (agent *Agent) deleteDeviceFromAdapters(ctx context.Context, forceOption bool, device *voltha.Device) coreutils.Response {
	response := coreutils.NewResponse()
	go func() {
		subCtx, cancel := context.WithTimeout(log.WithSpanFromContext(context.Background(), ctx),
		...
		if err != nil {
			...
			go agent.waitForAdapterFlowResponse(subCtx, cancel, ch, response)
		}
	}()
	return response
}
