...
func (agent *Agent) waitForAdapterResponseAndLog(ctx context.Context, cancel context.CancelFunc, rpc string, ch chan *kafka.RpcResponse,
...
func (agent *Agent) waitForAdapterResponse(ctx context.Context, cancel context.CancelFunc, rpc string, ch chan *kafka.RpcResponse,
	...
	defer func() {
		if rpce != nil {
			_ = agent.deviceMgr.RPCEventManager.SendRPCEvent(ctx, "RPC_ERROR_RAISE_EVENT", rpce,
...
func (agent *Agent) waitForAdapterFlowResponse(ctx context.Context, cancel context.CancelFunc, rpc string, ch chan *kafka.RpcResponse, response coreutils.Response) {
	...
	defer func() {
		if rpce != nil {
			_ = agent.deviceMgr.RPCEventManager.SendRPCEvent(ctx, "RPC_ERROR_RAISE_EVENT", rpce, voltha.EventCategory_COMMUNICATION, nil, time.Now().UnixNano())
		}
	}()
	select {
	case rpcResponse, ok := <-ch:
...
func (agent *Agent) deleteDeviceForce(ctx context.Context) error {
	...
	if previousAdminState != ic.AdminState_PREPROVISIONED {
		...
		go agent.waitForAdapterResponseAndLog(subCtx, cancel, "deleteDeviceForce", ch, agent.onSuccess,
			agent.onFailure)
	}
	return nil
}
