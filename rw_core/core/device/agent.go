...
func (agent *Agent) start(ctx context.Context, deviceExist bool, deviceToCreate *voltha.Device) (*voltha.Device, error) {
	...
	if deviceExist {
		...
		defer func() { agent.logDeviceUpdate(ctx, &prevState, &currState, operStatus, err, desc) }()
...
func (agent *Agent) onSuccess(ctx context.Context, prevState, currState *common.AdminState_Types, deviceUpdateLog bool) {
	if deviceUpdateLog {
		...
		agent.logDeviceUpdate(ctx, prevState, currState, operStatus, nil, desc)
		return
	}
...
func (agent *Agent) onFailure(ctx context.Context, err error, prevState, currState *common.AdminState_Types, deviceUpdateLog bool) {
	...
	if deviceUpdateLog {
		operStatus := &common.OperationResp{Code: common.OperationResp_OPERATION_FAILURE}
		desc := "adapter-response"
		agent.logDeviceUpdate(ctx, prevState, currState, operStatus, err, desc)
		return
	}
	logger.Debugw(ctx, "failed-operation", log.Fields{"device-id": agent.deviceID, "rpc": coreutils.GetRPCMetadataFromContext(ctx)})
}
...
func (agent *Agent) deleteDeviceForce(ctx context.Context) error {
	...
	if agent.isStateForceDeleting(previousDeviceTransientState) {
		agent.requestQueue.RequestComplete()
		err = status.Error(codes.FailedPrecondition, fmt.Sprintf("deviceId:%s, deletion is in progress", agent.deviceID))
		return err
	}
...
func (agent *Agent) simulateAlarm(ctx context.Context, simulateReq *voltha.SimulateAlarmRequest) error {
	...
	subCtx, cancel := context.WithTimeout(coreutils.WithAllMetadataFromContext(ctx), agent.rpcTimeout)
	opStatus.Code = common.OperationResp_OPERATION_IN_PROGRESS
	go func() {
		defer cancel()
...
func (agent *Agent) abortAllProcessing(ctx context.Context, device *voltha.Device) error {
	logger.Infow(ctx, "aborting-current-running-requests", log.Fields{"device-id": device.Id})
	if err := agent.requestQueue.WaitForGreenLight(ctx); err != nil {
		return err
	}
	defer agent.requestQueue.RequestComplete()
...
func (agent *Agent) ReconcileDevice(ctx context.Context, device *voltha.Device) {
	...
	if err := agent.requestQueue.WaitForGreenLight(ctx); err != nil {
		agent.logDeviceUpdate(ctx, nil, nil, operStatus, err, desc)
		return
	}
	...
	if err != nil {
		...
		agent.logDeviceUpdate(ctx, nil, nil, operStatus, nil, desc)
		return
	}
...
retry:
	for {
		...
		err := agent.sendReconcileRequestToAdapter(ctx, device, backoffTimer)
		if errors.Is(err, errContextExpired) || errors.Is(err, errReconcileAborted) {
			...
			operStatus = &common.OperationResp{Code: common.OperationResp_OperationReturnCode(common.OperStatus_FAILED)}
			desc = "aborted"
			agent.logDeviceUpdate(ctx, nil, nil, operStatus, err, desc)
			break retry
		}
...
func (agent *Agent) sendReconcileRequestToAdapter(ctx context.Context, device *voltha.Device, timer *time.Timer) error {
