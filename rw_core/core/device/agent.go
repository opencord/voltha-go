...
func (agent *Agent) updateDevice(ctx context.Context, config *voltha.UpdateDevice) error {
	...
	if err := agent.UpdateAddress(ctx, clonedDevice, config); err != nil {
		logger.Errorw(ctx, "update-device-ip-address-failed: ", log.Fields{"device-id": agent.deviceID, "error": err})
		agent.requestQueue.RequestComplete()
		return err
	}
...
func (agent *Agent) UpdateAddress(ctx context.Context, device *voltha.Device, deviceConfig *voltha.UpdateDevice) error {
	...
	case *voltha.UpdateDevice_Ipv4Address:
		if current, ok := device.Address.(*voltha.Device_Ipv4Address); ok {
			if current.Ipv4Address == addr.Ipv4Address {
				logger.Debugw(ctx, "device IPv4 address unchanged ", log.Fields{"device-id": device.Id, "ip": addr.Ipv4Address})
	...
	case *voltha.UpdateDevice_Ipv6Address:
		if current, ok := device.Address.(*voltha.Device_Ipv6Address); ok {
			if current.Ipv6Address == addr.Ipv6Address {
				logger.Debugw(ctx, "device IPv6 address unchanged ", log.Fields{"device-id": device.Id, "ip": addr.Ipv6Address})
				return errNoAddrChange
			}
	...
	case *voltha.UpdateDevice_HostAndPort:
		if current, ok := device.Address.(*voltha.Device_HostAndPort); ok {
			if current.HostAndPort == addr.HostAndPort {
				logger.Debugw(ctx, "device HostAndPort address unchanged ", log.Fields{"device-id": device.Id, "hostandport": addr.HostAndPort})
				return errNoAddrChange
			}
			device.Address = &voltha.Device_HostAndPort{HostAndPort: addr.HostAndPort}
		}
	default:
		return fmt.Errorf("invalid device config address type: %T", deviceConfig.Address)
	}
	return nil
}
