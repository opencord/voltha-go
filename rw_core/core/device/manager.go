...
func NewManagers(proxy *model.Proxy, adapterMgr *adapter.Manager, kmp kafka.InterContainerProxy, endpointMgr kafka.EndpointManager, corePairTopic, coreInstanceID string, defaultCoreTimeout time.Duration) (*Manager, *LogicalManager) {
	deviceMgr := &Manager{
		...
		dbProxy:                 proxy.SubProxy("devices"),
	...
	logicalDeviceMgr := &LogicalManager{
		...
		deviceMgr:                      deviceMgr,
		kafkaICProxy:                   kmp,
		dbProxy:                        proxy.SubProxy("logical_devices"),
...
func (dMgr *Manager) CreateDevice(ctx context.Context, device *voltha.Device) (*voltha.Device, error) {
	...
	agent := newAgent(dMgr.adapterProxy, device, dMgr, dMgr.dbProxy, dMgr.defaultTimeout)
...
func (dMgr *Manager) ListDevices(ctx context.Context, _ *empty.Empty) (*voltha.Devices, error) {
	...
	if err := dMgr.dbProxy.List(ctx, &devices); err != nil {
...
func (dMgr *Manager) getDeviceFromModel(ctx context.Context, deviceID string) (*voltha.Device, error) {
	...
	if have, err := dMgr.dbProxy.Get(ctx, deviceID, device); err != nil {
...
func (dMgr *Manager) ChildDeviceDetected(ctx context.Context, parentDeviceID string, parentPortNo int64, deviceType string,
	...
	// Create and start a device agent for that device
	agent := newAgent(dMgr.adapterProxy, childDevice, dMgr, dMgr.dbProxy, dMgr.defaultTimeout)
