...
func (dMgr *Manager) retryReconcilingRoutine(ctx context.Context, agent *Agent) {
	logger.Debugw(ctx, "retrying-reconciling", log.Fields{"deviceID": agent.deviceID})
	...
	if agent.getTransientState() != voltha.DeviceTransientState_RECONCILE_IN_PROGRESS {
		...
		if err != nil {
			logger.Errorf(ctx, "Not able to set device transient state to Reconcile in progress."+
				"Err: %s", err.Error())
		}
	}
...
Loop:
	for {
		...
		if !ok {
			//channel-closed
			logger.Errorf(ctx, "channel on which reconcile response is awaited is closed.")
		} else if resp.Err != nil {
			//error encountered
			logger.Errorw(ctx, "error encountered while retrying reconcile", log.Fields{"err": resp.Err})
		} else {
			err = agent.updateTransientState(ctx, voltha.DeviceTransientState_NONE)
			if err != nil {
				logger.Errorf(ctx, "Not able to clear device transient state from Reconcile in progress."+
					"Err: %s", err.Error())
			}
			break Loop
		}
