...
func (dMgr *Manager) adapterRestarted(ctx context.Context, adapter *voltha.Adapter) error {
	...
	for rootDeviceID := range dMgr.rootDevices {
		...
		if dAgent.deviceType == adapter.Type {
			...
			if rootDevice.AdapterEndpoint == adapter.Endpoint {
				if rootDevice.AdminState != voltha.AdminState_PREPROVISIONED {
					// Before reconciling, abort in-process request
					if err := dAgent.abortAllProcessing(utils.WithNewSpanAndRPCMetadataContext(ctx, "AbortProcessingOnRestart"), rootDevice); err != nil {
						logger.Errorw(ctx, "aborting-process-failed", log.Fields{"error": err, "device-id": rootDevice.Id})
						continue
					}
					logger.Debugw(ctx, "reconciling-root-device", log.Fields{"rootId": rootDevice.Id})
					go dAgent.ReconcileDevice(utils.WithNewSpanAndRPCMetadataContext(ctx, "ReconcileDevice"), rootDevice)
				} else {
				...
				for _, port := range rootDevicePorts {
					for _, peer := range port.Peers {
						if childDevice, _ := dMgr.getDeviceFromModel(ctx, peer.DeviceId); childDevice != nil {
							if childDevice.AdapterEndpoint == adapter.Endpoint {
								if childDevice.AdminState != voltha.AdminState_PREPROVISIONED {
									if err := dAgent.abortAllProcessing(utils.WithNewSpanAndRPCMetadataContext(ctx, "AbortProcessingOnRestart"), childDevice); err != nil {
										logger.Errorw(ctx, "aborting-process-failed", log.Fields{"error": err, "device-id": childDevice.Id})
										continue
									}
									logger.Debugw(ctx, "reconciling-child-device", log.Fields{"child-device-id": childDevice.Id})
									go dAgent.ReconcileDevice(utils.WithNewSpanAndRPCMetadataContext(ctx, "ReconcileDevice"), childDevice)
								} else {
...
func (dMgr *Manager) canMultipleAdapterRequestProceed(ctx context.Context, deviceIDs []string) error {
	...
	for _, deviceID := range deviceIDs {
		agent := dMgr.getDeviceAgent(ctx, deviceID)
		if agent == nil {
			logger.Errorw(ctx, "adapter-not-ready", log.Fields{"device-id": deviceID})
			return status.Errorf(codes.Unavailable, "adapter-not-ready-for-%s", deviceID)
		}
