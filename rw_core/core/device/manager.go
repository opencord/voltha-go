...
func (dMgr *Manager) UpdatePortsState(ctx context.Context, deviceID string, filter voltha.Port_PortType, state voltha.OperStatus_Types) error {
	logger.Debugw(ctx, "UpdatePortsState", log.Fields{"deviceid": deviceID})

	if agent := dMgr.getDeviceAgent(ctx, deviceID); agent != nil {
		switch state {
		case voltha.OperStatus_ACTIVE, voltha.OperStatus_UNKNOWN:
			if err := agent.updatePortsOperState(ctx, filter, state); err != nil {
		...
		if err := dMgr.logicalDeviceMgr.updatePortsState(ctx, device, filter, state); err != nil {
