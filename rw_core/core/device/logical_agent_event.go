...
func (agent *LogicalAgent) newRPCEvent(ctx context.Context, rpc, coreInstanceID, resourceID string) *voltha.RPCEvent {
	logger.Debugw(ctx, "newRPCEvent", log.Fields{"device-id": agent.logicalDeviceID})
	var opID string
	...
	rpcev := &voltha.RPCEvent{
		Rpc:         rpc,
		OperationId: opID,
		ResourceId:  agent.logicalDeviceID,
