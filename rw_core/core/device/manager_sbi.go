...
func (dMgr *Manager) ChildDeviceDetected(ctx context.Context, dd *ic.DeviceDiscovery) (*voltha.Device, error) {
	...
	childDevice.ProxyAddress = &voltha.Device_ProxyAddress{DeviceId: dd.ParentId, DeviceType: pAgent.deviceType, ChannelId: dd.ChannelId, OnuId: dd.OnuId, Endpoint: pEndPoint}
...
func (dMgr *Manager) ReconcileChildDevices(ctx context.Context, parentDeviceID *common.ID) (*empty.Empty, error) {
	...
	dAgent := dMgr.getDeviceAgent(ctx, parentDeviceID.Id)
	if dAgent == nil {
		return nil, status.Errorf(codes.NotFound, "parent device not found")
	}

	if parentDevicePorts, err := dMgr.listDevicePorts(ctx, parentDeviceID.Id); err == nil {
