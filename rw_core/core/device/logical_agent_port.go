...
func (agent *LogicalAgent) addNNILogicalPort(ctx context.Context, device *voltha.Device, port *voltha.Port) error {
	logger.Debugw(ctx, "addNNILogicalPort", log.Fields{"NNI": port})
	...
	if err != nil {
		logger.Errorw(ctx, "error-retrieving-port-capabilities", log.Fields{"error": err})
		return err
	}
	...
	go func() {
		...
		if err := agent.updateRoutes(context.Background(), device, newPort, agent.listLogicalDevicePorts(ctx)); err != nil {
			// This is not an error as we may not have enough logical ports to set up routes or some PON ports have not been
			// created yet.
			logger.Infow(ctx, "routes-not-ready", log.Fields{"logical-device-id": agent.logicalDeviceID, "logical-port": newPort.OfpPort.PortNo, "error": err})
		}

		// send event, and allow any queued events to be sent as well
		queuePosition.send(ctx, agent, agent.logicalDeviceID, ofp.OfpPortReason_OFPPR_ADD, newPort.OfpPort)
	}()
	return nil
}
...
func (agent *LogicalAgent) addUNILogicalPort(ctx context.Context, childDevice *voltha.Device, port *voltha.Port) error {
	...
	logger.Debugw(ctx, "adding-uni", log.Fields{"deviceId": childDevice.Id})
	newPort := portCap.Port
	...
	go func() {
		// First setup the routes
		if err := agent.updateRoutes(context.Background(), childDevice, newPort, agent.listLogicalDevicePorts(ctx)); err != nil {
			// This is not an error as we may not have enough logical ports to set up routes or some PON ports have not been
			// created yet.
			logger.Infow(ctx, "routes-not-ready", log.Fields{"logical-device-id": agent.logicalDeviceID, "logical-port": newPort.OfpPort.PortNo, "error": err})
		}
		// send event, and allow any queued events to be sent as well
		queuePosition.send(context.Background(), agent, agent.logicalDeviceID, ofp.OfpPortReason_OFPPR_ADD, newPort.OfpPort)
	}()
	return nil
}
