...
func (loader *Loader) Load(ctx context.Context) *voltha.DeviceReasons {
	loader.lock.Lock()
	defer loader.lock.Unlock()

	var reasons []*voltha.DeviceReason
	if err := loader.dbProxy.List(ctx, &reasons); err != nil {
		logger.Errorw(ctx, "failed-to-list-reasons-from-cluster-data-proxy", log.Fields{"error": err})
		return nil
	}
	return &voltha.DeviceReasons{Items: reasons}
}
...
// Update updates an device operation reason in the kv.
func (h *Handle) Update(ctx context.Context, reason *voltha.DeviceReason) error {

	if err := h.loader.dbProxy.Set(ctx, "", reason); err != nil {
		return status.Errorf(codes.Internal, "failed-reason-device-%v: %s", reason.DeviceId, err)
	}
