...
func (nb *NBTest) testDisableAndEnablePort(t *testing.T, nbi *APIHandler) {
	...
	var vdFunction isDeviceConditionSatisfied = func(device *voltha.Device) bool {
		for _, port := range device.Ports {
			if port.PortNo == cp.PortNo {
				return port.AdminState == voltha.AdminState_DISABLED && port.OperStatus == voltha.OperStatus_DISCOVERED
			}
		}
		return false
	}
	...
	vdFunction = func(device *voltha.Device) bool {
		for _, port := range device.Ports {
			if port.PortNo == cp.PortNo {
				return port.AdminState == voltha.AdminState_ENABLED && port.OperStatus == voltha.OperStatus_ACTIVE
			}
		}
		return false
	}
