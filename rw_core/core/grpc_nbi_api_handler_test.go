...
func (nb *NBTest) testDeviceUnreachableWhenOltIsEnabled(t *testing.T, nbi *APIHandler) {
	...
	err = waitUntilOltDeviceFlowsAreRemoved(oltDevice.Id, nb.maxTimeout, nbi, vlFunction2)
	assert.Nil(t, err)
	...
	err = waitUntilAllChilddDevicesAreRemoved(oltDevice.Id, nb.maxTimeout, nbi, vlFunction3)
	assert.Nil(t, err)
	...
	err = waitUntilLogicalDeviceIsCreated(oltDevice.Id, nb.maxTimeout, nbi, vlFunction4)
	assert.Nil(t, err)
