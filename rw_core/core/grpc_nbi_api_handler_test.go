...
func newNBTest() *NBTest {
	...
	test.defaultTimeout = 30 * time.Second
	test.maxTimeout = 60 * time.Second
	return test
}
...
func (nb *NBTest) monitorLogicalDevice(t *testing.T, nbi *APIHandler, numNNIPorts int, numUNIPorts int, wg *sync.WaitGroup) {
	...
	log.Error("About to UpdateLogicalDeviceMeter")
	...
	log.Errorw("About to listen for port events", log.Fields{"ceq": nbi.changeEventQueue})

	// Listen for port events
	processedLogicalPorts := 0
	for event := range nbi.changeEventQueue {
		log.Errorw("top of changeEventQueue loop", log.Fields{"event": event})
		startingVlan++
	...
	log.Error("Outside of changeEventQeue loop")
