...
func registerAdapterRequestHandlers(ctx context.Context, kmp kafka.InterContainerProxy, dMgr *device.Manager, aMgr *adapter.Manager, coreTopic string) {
	...
	if err := kmp.SubscribeWithDefaultRequestHandler(ctx, kafka.Topic{Name: coreTopic}, kafka.OffsetNewest); err != nil {
		logger.Fatalw(ctx, "Failed-registering-pair-handler", log.Fields{"topic": coreTopic})
	}
