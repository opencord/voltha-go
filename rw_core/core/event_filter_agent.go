...
func (efa *EventFilterAgent) start(ctx context.Context) {
	log.Debug("starting-event-filter-agent")
	efa.filterProxy = efa.modelProxy.CreateProxy(context.Background(), "/event_filters", false)
}
...
func (efa *EventFilterAgent) CreateEventFilter(filter *voltha.EventFilter) (*voltha.EventFilter, error) {
	...
	efa.modelProxy.Add(context.TODO(), "/event_filters", filter, "")
		...
		for event, filter := range device {
			if filter.Id == filterID.Id {
				...
				if agent := efa.deviceMgr.getDeviceAgent(filter.DeviceId); agent != nil {
					...
					efa.modelProxy.Remove(context.TODO(), "/event_filters/"+filterID.Id, "")
				}
				delete(device, event)
				break
			}
	...
	for id, device := range efa.filters {
		if id == deviceID.Id {
			...
			for _, filter := range device {
				...
				efa.modelProxy.Remove(context.TODO(), "/event_filters/"+filter.Id, "")
			}
