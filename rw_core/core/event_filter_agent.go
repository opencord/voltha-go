...
const (
	...
	BasePathKvStore = "service/voltha/openolt/event_filters"
	// FilterPathSuffix - filter-type-<type-of-filter>
	FilterPathSuffix = "filter-type-%s"
...
func InitKVClient(host string, port int, storeType string) (*db.Backend, error) {
	...
	kvbackend := &db.Backend{
		...
		PathPrefix: fmt.Sprintf(BasePathKvStore)}
...
func (efa *EventFilterAgent) CreateEventFilter(filter *voltha.EventFilter) error {
	...
	path := fmt.Sprintf("%s/"+FilterPathSuffix, filter.DeviceId, filter.EventType)
...
func (efa *EventFilterAgent) UpdateEventFilter(newFilter *voltha.EventFilter) error {
	...
	path := fmt.Sprintf("%s/"+FilterPathSuffix, oldFilter.DeviceId, oldFilter.EventType)
	...
	for id, device := range efa.filters {
		if id == deviceId.Id {
			...
			for _, filter := range device {
				path := fmt.Sprintf("%s/"+FilterPathSuffix, id, filter.EventType)
				if err := efa.KVStore.Delete(path); err != nil {
