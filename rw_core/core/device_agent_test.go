...
func (dat *DATest) updateDeviceConcurrently(t *testing.T, da *DeviceAgent, globalWG *sync.WaitGroup) {
	originalDevice, err := da.getDevice()
	assert.Nil(t, err)
	...
	updatedDevice, err := da.getDevice()
	assert.Nil(t, err)
	assert.NotNil(t, updatedDevice)
	assert.Equal(t, expectedChange.String(), updatedDevice.String())
