...
func (ap *AdapterProxy) startOmciTest(ctx context.Context, device *voltha.Device, omcitestrequest *voltha.OmciTestRequest) (*voltha.TestResponse, error) {
	...
	// Use a device specific topic as we are the only core handling requests for this device
	replyToTopic := ap.getCoreTopic()
	success, result := ap.kafkaICProxy.InvokeRPC(ctx, rpc, &toTopic, &replyToTopic, true, device.Id, &kafka.KVArg{"device", device}, &kafka.KVArg{"omcitestrequest", omcitestrequest})
	...
	if success {
		...
		if err := ptypes.UnmarshalAny(result, unpackResult); err != nil {
			return nil, status.Errorf(codes.InvalidArgument, "cannot-unmarshall-test-response: %s", err.Error())
		}
		log.Debugw("omci-test-response-adpater-proxy", log.Fields{"deviceId": device.Id, "unpackResult": unpackResult.Result, "result": result})
		return unpackResult, nil
	}
	return nil, unPackResponse(rpc, device.Id, success, result)
}
