...
func (ap *AdapterProxy) getValue(ctx context.Context, device *voltha.Device, id *voltha.ID, valuetype voltha.ValueType_Type) (chan *kafka.RpcResponse, error) {
	log.Debugw("getValue", log.Fields{"device-id": device.Id, "onuid": id})
	rpc := "get_value"
	toTopic, err := ap.getAdapterTopic(device.Id, device.Adapter) // XXX smbaker
	if err != nil {
		return nil, err
	}
