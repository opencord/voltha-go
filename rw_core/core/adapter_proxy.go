...
func (ap *AdapterProxy) getAdapterTopic(deviceId string, adapterName string) (*kafka.Topic, error) {

	endpoint, err := ap.endpointManager.GetEndpointByService(deviceId, adapterName)
	if err != nil {
		return nil, err
	}
...
func (ap *AdapterProxy) childDeviceLost(ctx context.Context, deviceId string, deviceType string, pDeviceID string, pPortNo uint32, onuID uint32) (chan *kafka.RpcResponse, error) {
