...
func (cf *RWCoreFlags) ParseCommandArguments() {
	...
	flag.Var(&addressValue{&(cf.GrpcAddress)}, "grpc_address", help)

	help = fmt.Sprintf("Kafka - Adapter messaging address")
	flag.Var(&addressValue{&(cf.KafkaAdapterAddress)}, "kafka_adapter_address", help)

	help = fmt.Sprintf("Kafka - Cluster messaging address")
	flag.Var(&addressValue{&(cf.KafkaClusterAddress)}, "kafka_cluster_address", help)
	...
	flag.Var(&addressValue{&(cf.KVStoreAddress)}, "kv_store_address", help)
	...
	flag.Var(&addressValue{&(cf.ProbeAddress)}, "probe_address", help)
