...
var stringToRType = map[string]routeType{"rpc_affinity_message": RouteTypeRpcAffinityMessage, "rpc_affinity_header": RouteTypeRpcAffinityHeader, "binding": RouteTypeBinding, "round_robin": RouteTypeRoundRobin}
var rTypeToString = map[routeType]string{RouteTypeRpcAffinityMessage: "rpc_affinity_message", RouteTypeRpcAffinityHeader: "rpc_affinity_header", RouteTypeBinding: "binding", RouteTypeRoundRobin: "round_robin"}

func (t routeType) MarshalJSON() ([]byte, error) {
	if t == RouteTypeUndefined {
		return json.Marshal(nil)
	}
	if str, have := rTypeToString[t]; have {
		return json.Marshal(str)
	}
	return nil, fmt.Errorf("unknown %T '%d'", t, t)
}
...
func (t *routeType) UnmarshalJSON(b []byte) error {
	...
	var have bool
	if *t, have = stringToRType[str]; !have {
		return fmt.Errorf("invalid %T %s", *t, str)
	}
	return nil
}
