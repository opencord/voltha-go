...
// forwardClientToServer forwards the response stream
func (r *request) forwardClientToServer(connName string, stream grpc.ClientStream) {
	...
	for {
		...
		r.activeStreamOnce.Do(func() { activeStream = true })
		if err != nil {
...
func (r *request) sendResponseFrame(stream grpc.ClientStream, f responseFrame) error {
	r.serverStreamMutex.Lock()
	defer r.serverStreamMutex.Unlock()

	// the header should only be set once, even if multiple streams can respond.
	setHeader := false
	r.setHeaderOnce.Do(func() { setHeader = true })
	if setHeader {
...
func (r *request) sendAll(frameBacklog *[][]byte, frame *requestFrame) error {
	// only send to existing connections
	r.mutex.Lock()
	catchupStreams := make(map[grpc.ClientStream]struct{}, len(r.newStreams))
	for n := range r.newStreams {
		catchupStreams[n] = struct{}{}
		delete(r.newStreams, n)
	}
	...
	tmp := frame.payload
	for stream := range catchupStreams {
		for _, payload := range *frameBacklog {
			frame.payload = payload
	...
	if !r.isStreamingRequest {
		// save frames of non-streaming requests, so we can prime new streams
		*frameBacklog = append(*frameBacklog, frame.payload)
	}
...
func (r *request) forwardServerToClients(src grpc.ServerStream, frame *requestFrame) error {
	...
	// then read some more.
	var rtrn error
	var frameBacklog [][]byte
	for {
		// Send the message to each of the backend streams
		if err := r.sendAll(&frameBacklog, frame); err != nil {
			log.Debugf("SendAll failed %s", err.Error())
			rtrn = err
			break
		}
		log.Debugf("Southbound frame %s", hex.EncodeToString(frame.payload))
