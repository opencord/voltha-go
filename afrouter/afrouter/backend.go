...
func (be *backend) splitActiveStreamsUnsafe(cn *connection, conn *grpc.ClientConn) {
	...
	for r := range be.activeRequests {
		...
		if _, have := r.streams[cn.name]; !have {
			...
			if stream, err := grpc.NewClientStream(r.ctx, clientStreamDescForProxying, conn, r.method); err != nil {
				log.Debugf("Failed to create a client stream '%s', %v", cn.name, err)
			} else {
				r.streams[cn.name] = stream
				if !r.isStreamingRequest {
					r.newStreams[stream] = struct{}{}
				}
				go r.forwardClientToServer(cn.name, stream)
			}
		}
		r.mutex.Unlock()
	}
...
var streamingRequest = map[string]struct{}{"StreamPacketsOut": {}}
var streamingResponse = map[string]struct{}{"ReceivePacketsIn": {}, "ReceiveChangeEvents": {}}
...
func (be *backend) openSouthboundStreams(srv interface{}, serverStream grpc.ServerStream, nf *requestFrame, sf *responseFrame) (*request, error) {
	...
	_, isStreamingRequest := streamingRequest[nf.methodInfo.method]
	_, isStreamingResponse := streamingResponse[nf.methodInfo.method]
	...
	r := &request{
		...
		newStreams:      make(map[grpc.ClientStream]struct{}),
		responseErrChan: make(chan error, 1),

		backend:             be,
		serverStream:        serverStream,
		method:              nf.methodInfo.all,
		responseFrame:       sf,
	...
	log.Debugf("There are %d/%d connections to open", len(be.openConns), len(be.connections))
...
func (be *backend) handler(srv interface{}, serverStream grpc.ServerStream, nf *requestFrame, sf *responseFrame) error {
	...
	if s2cErr := request.forwardServerToClients(serverStream, nf); s2cErr != nil {
